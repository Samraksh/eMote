...
var g_ServerEcdhpIP *EcdhProtoIP
var g_ClientEcdhpIP *EcdhProtoIP

func NewEcdhProtoIP(eccsize uint16) *EcdhProtoIP {

	fmt.Println("NewEcdhProtoIP::Creating new ECDH instance")
	if eccsize != 384 {
		return nil
	}
	tp384 := elliptic.P384()
...
func (dhp *EcdhProtoIP) Initiate() []byte {
	var bmsg []byte
	g_ClientEcdhpIP = dhp
...
func (dhp *EcdhProtoIP) CreateRequest() []byte {
...
func (dhp *EcdhProtoIP) ReqAck(sessionNo uint16, ecc_curve_size uint16, publickey []byte, msg [128]byte, nonce uint64) {
...
func (dhp *EcdhProtoIP) CreateResponse(req *EcdhpRequestS) []byte {
...
func (dhp *EcdhProtoIP) CreateFinalize(res *EcdhpResponseS) []byte {
...
/*func (dhp *EcdhProtoIP) SendFinalize(res *EcdhpResponseS) {

}*/

///Other methods
func (dhp *EcdhProtoIP) ComputeSecret1(peerPub crypto.PublicKey) []byte {
...
func (dhp *EcdhProtoIP) ComputeSecret() []byte {
	//fmt.Println("Peer public key:", dhp.PeerPubKey)
...
func (dhp *EcdhProtoIP) GetPublicKey() []byte {
	var pubKey []byte
...
func (dhp *EcdhProtoIP) PutPeerPublicKey(mpub []byte) {
	//var pubKey []byte
...
func EcdhpIP_StateMachine(msg []byte, dataC *chan Def.IPMsg, inaddr net.Addr) {
	var outB []byte
	ret := UnMarshall(msg)
	switch ret := ret.(type) {
	default:
	...
	case *EcdhpRequestS:
		...
		g_ServerEcdhpIP = NewEcdhProtoIP(reqS.Ecc_curve_size)
		outB = g_ClientEcdhpIP.CreateResponse(reqS)
	...
	case *EcdhpResponseS:
		...
		if GSM.IsPresent(resS.SessionNo) == -1 {
			...
			outB = g_ClientEcdhpIP.CreateFinalize(resS)
