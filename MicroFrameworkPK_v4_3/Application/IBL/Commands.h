...
#define COMMAND_HEADER()               HRESULT hr
#define COMMAND_RETURN() return hr
#define COMMAND_CHECK_HRESULT(expr)    { if(FAILED(hr = (expr))) COMMAND_LEAVE(); }
#define COMMAND_EXIT_ON_SUCCESS(expr)  { if(SUCCEEDED(hr = (expr))) COMMAND_LEAVE(); }
#define COMMAND_SET_AND_LEAVE(expr)    { hr = (expr); COMMAND_LEAVE(); }  // EXAMPLE DEBUG CLR ERRORS: { hr = (expr); if(hr==CLR_E_WRONG_TYPE){SOFT_BREAKPOINT();} COMMAND_LEAVE(); }
#define COMMAND_CLEANUP()              hr = S_OK; COMMAND_Cleanup:
#define COMMAND_CLEANUP_END()          COMMAND_RETURN()
#define COMMAND_NOCLEANUP()            COMMAND_CLEANUP(); COMMAND_CLEANUP_END()
#define COMMAND_NOCLEANUP_NOLABEL()    hr = S_OK; COMMAND_RETURN()
#define FAULT_ON_NULL(ptr)             if(!(ptr)) COMMAND_SET_AND_LEAVE(CLR_E_NULL_REFERENCE)
#define FAULT_ON_NULL_ARG(ptr)         if(!(ptr)) COMMAND_SET_AND_LEAVE(CLR_E_ARGUMENT_NULL)
