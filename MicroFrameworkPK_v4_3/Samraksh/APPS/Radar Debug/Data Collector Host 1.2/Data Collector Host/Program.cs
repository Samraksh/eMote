 ...
 *      1.2 
 *          -   Fixed issue wrt report on bytes received
 ***********************************************************/
...
namespace Samraksh.AppNote.DataCollector.Host {
    ...
    internal class Program {
        ...
        private static void Main() {
            ...
            var fileSizeUshorts = _byteCntr / sizeof(ushort);
            ConsoleDebugWriteLine("\n" + _byteCntr + " bytes, " + fileSizeUshorts + " ushorts, " + fileSizeUshorts / DataCollectBufferSize + " buffers");
        ...
        private static void WriteFile(byte[] buffer, int actualLength) {
            ...
            for (var i = 0; i < actualLength; i += 2) {
                if (buffer[i] != Eof || buffer[i + 1] != Eof) {
                    _byteCntr += 2;
                    continue;
                }
