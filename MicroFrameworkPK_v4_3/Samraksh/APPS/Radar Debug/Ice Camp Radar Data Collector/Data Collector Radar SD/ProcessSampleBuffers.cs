...
enum DETECTOR
{
    ...
    THRESH = 60,
    DC_EST_SECS = 3,
    ADC_BUFF_ANALYSIS_LENGTH = 256      // We don't have time to sort and process the entire ADC buffer because the ADC callback gets called before processing is done so we only analyze a part of the buffer
}
...
namespace Samraksh.AppNote.DataCollector.Radar
{
    ...
    public partial class Program
    {
        ...
        private static void SetupADCBuffers()
        {
            ...
            radarDetect.SetDetectionParameters(65,0);
            Counter.count = 0;
        ...
        private static void SampleBufferQueue()
        {
            ...
            while (true)
            {
                ...
                AnalogInput.StopSampling();
                LargeDataRef.Flush(ADCBufferSize, 0);
        ...
        private static void ProcessBuffers(IQAud iqa)
        {
            ...
            //threshholdMet = pUn.DetectionTest(unwrapBuffer, ADCBufferSize);
            ...
            for (i = 0; i < ADCBufferSize; i++)
            {
                ...
                iqBuffer[(i * 3)] = iBuff[i];
                iqBuffer[(i * 3) + 1] = qBuff[i];
                /*if (threshholdMet == true)
                    iqBuffer[(i * 3)] = (ushort)1;
                else
                    iqBuffer[(i * 3)] = (ushort)0;
                iqBuffer[(i * 3) + 1] = (ushort)MoutOfNDetector.state;*/
                iqBuffer[(i * 3) + 2] = unwrapBuffer[i];

                //if ((int)MoutOfNDetector.state == 1)
                if (threshholdMet == true)
                {
            ...
            //Debug.Print("mean I:" + DCEstimation.mean.I.ToString() + " Q:" + DCEstimation.mean.Q.ToString()); 
            //EnhancedLcd.Display(DCEstimation.mean.I.ToString());


            LargeDataRef.Write(iqBuffer);
            if (_channelBuffersProcessedCtr++ > 100)
            {
                _collectIsDone = true;
            }
        ...
        public static void initializeVariables()
        {
            ...
            Counter.count = 0;
            //Debug.Print("Initialized variables");
        }
