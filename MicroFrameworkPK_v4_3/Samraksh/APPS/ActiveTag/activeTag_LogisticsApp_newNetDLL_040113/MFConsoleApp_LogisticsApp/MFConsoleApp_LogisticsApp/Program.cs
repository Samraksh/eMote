...
/** ACTIVE TAG SNIPPET CONSTRUCTION and TEST **/

//ASSUMPTION 1: There is a routing layer that is invoked when a message payload is to be sent out.
//  ASSUMPTION 1a: The routing layer talks to the MAC for sending 
//
//ASSUMPTION 2: IT is fact that the MAC API provides handlers for receiving messages and ACKS.
//  BUT it is assumed that a simple 'forwarding' that is not meant for the recepient will not propagate 
//      out of the MAC layer.
//ASSUMPTION 2 MAY NOT BE TRUE MAY HAVE TO BE HANDLED
//
// FROM An Application Layer, the Message Destination is:
//   To Base Station, To Specific Node, To Neighbor Nodes and To All.  
//   It is unlikely that a specific node will want to send a msg with a destination ToAll.
//  From the App Layer, the route to the Message Destination is handled by the Routing and then MAC layers.
//     The App Layer knows nothing of the route (and knows nothing of the concept of Forward).

namespace ActiveTag_LogisticsApp_VerifiedMsgPayload_NewDLL040113
{
   ...
   */
    ...
    */
    ...
    public class LogisticsApp
    {
        ...
        static Samraksh.SPOT.Net.Mac.CSMA myCSMAMac = new CSMA();
        ...
        static UInt16 maxRecheckCycle = 500;  //originally set at 5000 but reset for testing
        ...
        void init()
        {
            ...
            try
            {
                myCSMAMac.Initialize(macConfig, myReceive);
            }
            catch (Exception e)
            {
            ...
            while (true)
            {
                ...
                if (la.shouldProcessThisMessage == true)
                {
                    ...
                    if (la.theMsgDestType == MessageDestinationType.BaseStation)
                    {
                        ...
                        theNetworking.sendingMsg(la.mp.getMessagePayloadBuffer(), la.mp.getByteOffset(), (UInt16)la.mp.getLengthOfEntireMessage(), myCSMAMac);
                    }
        ...
        */
        ...
        void forwardMsgToSpecificNode(MessagePayload theMP)
        {
            ...
            theNetworking.sendingMsg(theMP.getMessagePayloadBuffer(), theMP.getByteOffset(), (UInt16)theMP.getLengthOfEntireMessage(), myCSMAMac);
        ...
        */
        ...
        //void MsgsReceived(byte[] msgIn, ushort msgSize)
        {
            ...
            Debug.Print(" In Msgs Received:  should not be here now with no incoming msgs -- nothing set.");
              ...
              recvMP.setLengthOfEntireMessage(msgSize);
              recvMP.setPayloadFieldsFromMsgByteArray(msgIn, msgSize);
