...
UINT64 SDInternal::InternalTotalSize( HRESULT &hr )
{
    UINT64 retVal = 0; 
    return retVal;
}

UINT64 SDInternal::InternalTotalFreeSpace( HRESULT &hr )
{
    UINT64 retVal = 0; 
    return retVal;
}

INT32 SDInternal::InternalSize( long long int totalSize, long long int totalFreeSpace, HRESULT &hr )
{
	FileSystemVolume* pFSVolume;
	pFSVolume = FileSystemVolumeList::FindVolume("U", 1);
	totalSize = 0, totalFreeSpace = 0;
