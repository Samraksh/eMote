...
INT32 MACBase::InternalInitialize( CLR_RT_HeapBlock* pMngObj, CLR_RT_TypedArray_UINT8 param0, UINT8 param1, HRESULT &hr )
{
	...
	{
		hal_printf("\nMACBase.cpp: Mac_Initialize failed \n");
		return DS_Fail;
	}

	//hal_printf("\nMACBase.cpp: After Mac_Initialize\n");
	if(CPU_Radio_ChangeTxPower( configParams[11], configParams[9]) != DS_Success)
	{
		hal_printf("\nMACBase.cpp: CPU_Radio_ChangeTxPower failed \n");
		return DS_Fail;
	}

	//hal_printf("\nMACBase.cpp: After CPU_Radio_ChangeTxPower\n");
	if(CPU_Radio_ChangeChannel( configParams[11], configParams[10]) != DS_Success)
	{
		hal_printf("\nMACBase.cpp: CPU_Radio_ChangeChannel failed \n");
		return DS_Fail;
	}
	//hal_printf("\nMACBase.cpp: After CPU_Radio_ChangeChannel\n");
