...
//#ifdef PLATFORM_ARM_KRAIT
//#include <Targets\Native\Krait\DeviceCode\Krait_SPI\Krait_SPI.h>

#define RF231_SEL_PIN	(GPIO_PIN)58
#define RF231_RST_PIN	(GPIO_PIN)55
#define RF231_SLP_PIN	(GPIO_PIN)53
#define RF231_AMP_PIN	(GPIO_PIN)52

//#elif PLATFORM_ARM_EmoteDotNow
#ifdef PLATFORM_ARM_EmoteDotNow
#include <Targets\Native\STM32F10x\DeviceCode\drivers\spi\netmf_spi.h>

#define RF231_SEL_PIN	GPIO_Pin_9
#define RF231_RST_PIN	GPIO_Pin_10
#define RF231_SLP_PIN	GPIO_Pin_11
#define RF231_AMP_PIN	GPIO_Pin_10
#endif
//AnanthAtSamraksh
...
#define RF231_SEL(x) { if(x) GPIO_WriteBit(GPIOF, RF231_SEL_PIN, Bit_SET); else GPIO_WriteBit(GPIOF, RF231_SEL_PIN, Bit_RESET); }
#define RF231_RST(x) { if(x) GPIO_WriteBit(GPIOF, RF231_RST_PIN, Bit_SET); else GPIO_WriteBit(GPIOF, RF231_RST_PIN, Bit_RESET); }
#define RF231_SLP(x) { if(x) GPIO_WriteBit(GPIOB, RF231_SLP_PIN, Bit_SET); else GPIO_WriteBit(GPIOB, RF231_SLP_PIN, Bit_RESET); }
#define RF231_AMP(x) { if(x) GPIO_WriteBit(GPIOB, RF231_AMP_PIN, Bit_SET); else GPIO_WriteBit(GPIOB, RF231_AMP_PIN, Bit_RESET); }
...
//AnanthAtSamraksh - the objects have to be externed in header and defined in a cpp file, so that compiler knows that objects are already declared elsewhere.
extern RF231Radio grf231Radio, grf231RadioLR;
