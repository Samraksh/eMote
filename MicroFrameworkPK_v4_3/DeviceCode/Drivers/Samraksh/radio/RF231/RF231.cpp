...
void RF231Radio::WriteRegister(UINT8 reg, UINT8 value)
{
	...
	//writel(value, reg);
	////*((volatile UINT32 *)(RF230_CMD_REGISTER_WRITE | reg)) = value;
...
UINT8 RF231Radio::ReadRegister(UINT8 reg)
{
	...
	//read_reg = *((volatile UINT32 *)(RF230_CMD_REGISTER_READ | reg));
...
DeviceStatus RF231Radio::ChangeTxPower(int power)
{
	...
	{
		hal_printf("\nRF231.cpp: failure ChangeTxPower %d \n", state);
		//return DS_Fail;
	}

	if(state == STATE_SLEEP)
	{
		hal_printf("\nRF231.cpp: state is STATE_SLEEP -- %d \n", state);
	...
	if(sleep_pending == TRUE)
	{
		...
		{
			sleep_pending = TRUE;
			hal_printf("\nRF231.cpp: RF231Radio::ChangeTxPower. sleep(0) failed \n");
			return DS_Fail;
		}
		sleep_pending == FALSE;
	}
...
UINT8  rf231_read_reg(UINT8 reg) {
	UINT8 ret;
	RF231_SEL(0);
...
UINT8  rf231_write_reg(UINT8 reg, UINT8 data) {
	UINT8 ret;
	volatile int i;
	RF231_SEL(0);
	while (SPI_I2S_GetFlagStatus(RF231_SPI, SPI_I2S_FLAG_TXE) == RESET) { ; }
...
void RF231Radio::rf231_enable_pa_rxtx(void) {
	volatile int i;
	UINT8 data = ReadRegister(RF231_REG_TX_CTRL_1);
	for (i=0; i<10000; i++) { ; } // wait
	//UINT8 data = rf231_read_reg(RF231_REG_TX_CTRL_1);
	data |= 0x80;
	WriteRegister(RF231_REG_TX_CTRL_1, data);
	for (i=0; i<10000; i++) { ; } // wait
...
DeviceStatus RF231Radio::Initialize(RadioEventHandler *event_handler, UINT8 radio, UINT8 mac_id)
{
	...
	if(!IsInitialized())
	{
		...
		//hal_printf("\nRF231.cpp: after GpioPinInitialize\n");
		if(TRUE != SpiInitialize())
		{
			ASSERT_RADIO("SPI Initialization failed");
		}

		//hal_printf("\nRF231.cpp: after SpiInitialize\n");
		...
		//AnanthAtSamraksh - testing if able to get right values from registers
		UINT8 read_reg;
		read_reg = ReadRegister(0x1C);
		read_reg = ReadRegister(0x1D);
		read_reg = ReadRegister(0x1E);
		//AnanthAtSamraksh - testing if able to get right values from registers
...
#endif
		//hal_printf("\nRF231.cpp: Initialize: rf230_trx_off %d\n", RF230_TRX_OFF);
		DID_STATE_CHANGE(RF230_TRX_OFF);
...
DeviceStatus RF231Radio::TurnOn()
{
	...
	if(state != STATE_SLEEP)
	{
		hal_printf("\nRF231.cpp: TurnOn. state %d \n", state);
		return DS_Success;
	}

	if(this->GetRadioName() == RF231RADIOLR)
	{
		//hal_printf("\nRF231.cpp: TurnOn. setting AntDiversity \n");
	...
	//hal_printf("\nRF231.cpp: TurnOn: RF230_TRX_OFF %d\n", RF230_TRX_OFF);
	DID_STATE_CHANGE(RF230_TRX_OFF);
	...
	//hal_printf("\nRF231.cpp: TurnOn: RF230_RX_ON %d\n", RF230_RX_ON);
	DID_STATE_CHANGE(RF230_RX_ON);
...
RF231Radio grf231Radio, grf231RadioLR;
