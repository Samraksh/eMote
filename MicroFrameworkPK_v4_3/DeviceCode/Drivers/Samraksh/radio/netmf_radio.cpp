...
DeviceStatus CPU_Radio_Initialize(RadioEventHandler* eventHandlers, UINT8 radioID, UINT8 numberRadios, UINT8 mac_id )
{
	...
	{
		case RF231RADIO:
			//hal_printf("netmf_radio.cpp: grf231Radio.Initialize \n");
			status = grf231Radio.Initialize(eventHandlers, radioID, mac_id);
			break;
		case RF231RADIOLR:
			//hal_printf("netmf_radio.cpp: grf231RadioLR.Initialize \n");
...
DeviceStatus CPU_Radio_TurnOn(UINT8 radioID)
{
	...
	switch(radioID)
	{
		case RF231RADIO:
			...
			break;
		case RF231RADIOLR:
			//hal_printf("\nnetmf_radio.cpp: CPU_Radio_TurnOn. calling LR TurnOn \n");
...
DeviceStatus CPU_Radio_ChangeTxPower(UINT8 radioID, int power)
{
	...
	{
		case RF231RADIO:
			//hal_printf("netmf_radio.cpp: calling grf231Radio.ChangeTxPower -- %d \n", power);
			status = grf231Radio.ChangeTxPower(power);
			break;
		case RF231RADIOLR:
			//hal_printf("\nnetmf_radio.cpp: calling grf231RadioLR.ChangeTxPower -- %d \n", power);
