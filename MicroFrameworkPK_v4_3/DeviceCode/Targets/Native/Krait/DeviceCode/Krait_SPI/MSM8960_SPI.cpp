...
#include <MSM8960_SPI.h>

MSM8960_SPI_Driver MSM8960_SPI_Driver_obj;
...
static void SPI_print_status(const SPI_config &config)
{
	...
	printf("QUP_CONFIG is :0x%x\n", value);
	value = readl(QUP_STATE(config.gsbi_id));
	printf("QUP_STATE is :0x%x\n", value);
	value = readl(QUP_IO_MODES(config.gsbi_id));
	printf("QUP_IO_MODES is :0x%x\n", value);
}
#endif
...
void MSM8960_SPI_Driver::spi_write_bit(UINT8 data)
{
...
UINT8* MSM8960_SPI_Driver::spi_read_bit()
{
	UINT8* data;
...
void MSM8960_SPI_Driver::spi_write_byte(UINT32 data)
{
...
UINT32* MSM8960_SPI_Driver::spi_read_byte()
{
	UINT32* data;
	data = gpio_get(SPI_GPIO_MISO);
	return data;
...
*/
BOOL MSM8960_SPI_Driver::Initialize(int gsbi_id, int SPI_mod, int IO_mode, int master_slave_mode, int clock_rate)
{
	UINT32 l; // Temp var
	UINT32 i; // Timeout

	printf("Inside SPI_Driver Initialize function \n");
...
void MSM8960_SPI_Driver::Uninitialize(const SPI_config &config)
{
	/* Step 2 in 16.4.4.1:
...
void MSM8960_SPI_Driver::ISR( void* Param )
{
    ASSERT(0);
}

BOOL MSM8960_SPI_Driver::nWrite16_nRead16( const SPI_config& config, UINT16* Write16, INT32 WriteCount, UINT16* Read16, INT32 ReadCount, INT32 ReadStartOffset )
{
...
BOOL MSM8960_SPI_Driver::nWrite8_nRead8( const SPI_config& config, UINT8* Write8, INT32 WriteCount, UINT8* Read8, INT32 ReadCount, INT32 ReadStartOffset )
{
...
/*
	...
	Returns TRUE.
*/
BOOL MSM8960_SPI_Driver::Xaction_Start( const SPI_config& config)
{
...
BOOL MSM8960_SPI_Driver::Xaction_Stop( const SPI_config& Configuration )
{
	// Enable PAUSE_STATE
...
*/
BOOL MSM8960_SPI_Driver::Xaction_nWrite16_nRead16( SPI_XACTION_16& Transaction )
{
    INT32 i;
    INT32 d;
    UINT16 Data16;
...
/*
	...
	Returns FALSE on error
*/
BOOL MSM8960_SPI_Driver::Xaction_nWrite8_nRead8( SPI_XACTION_8& Transaction )
{
    INT32 i;
    INT32 d;
    UINT8 Data8;
...
/*
	Not used.
*/
void MSM8960_SPI_Driver::GetPins( UINT32 spi_mod, GPIO_PIN& msk, GPIO_PIN& miso, GPIO_PIN& mosi )
{
