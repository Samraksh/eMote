...
void SX1276M1BxASWrapper::Write(uint8_t addr, uint8_t data) {
	 SX1276M1BxASWrapper::Write( addr, &data, 1 );
}

uint8_t SX1276M1BxASWrapper::Read(uint8_t addr) {
    uint8_t data;
    SX1276M1BxASWrapper::Read( addr, &data, 1 );
    return data;
}
...
void SX1276M1BxASWrapper::Read(uint8_t addr, uint8_t* buffer,
	...
	SX1276M1BxASWrapper::radio_comm_ReadData(addr & 0x7F, 0, size, buffer);
}

void SX1276M1BxASWrapper::WriteFifo(uint8_t* buffer,
		uint8_t size) {
	SX1276M1BxASWrapper::Write( 0, buffer, size );
}

void SX1276M1BxASWrapper::ReadFifo(uint8_t* buffer,
		uint8_t size) {
	SX1276M1BxASWrapper::Read( 0, buffer, size );
}



void SX1276M1BxASWrapper::Reset() { //TODO: B
	//SX1276M1BxASWrapper::Write a zero to the reset pin
...
void SX1276M1BxASWrapper::RadioRegistersInit() {
    ...
    for( i = 0; i < sizeof( RadioRegsInit ) / sizeof( RadioRegisters_t ); i++ )
    {
        ...
        SX1276M1BxASWrapper::Write( RadioRegsInit[i].Addr, RadioRegsInit[i].Value );
    }
}
...
void SX1276M1BxASWrapper::Initialize(SX1276RadioEvents_t *events) {
	...
	SX1276M1BxASWrapper::InitializeTimers();
	SX1276M1BxASWrapper::Init(events);

	SX1276M1BxASWrapper::SetModem( MODEM_FSK );
...
void SX1276M1BxASWrapper::SetChannel( uint32_t freq )
{
    ...
    SX1276M1BxASWrapper::Write( REG_FRFMSB, ( uint8_t )( ( freq >> 16 ) & 0xFF ) );
    SX1276M1BxASWrapper::Write( REG_FRFMID, ( uint8_t )( ( freq >> 8 ) & 0xFF ) );
    SX1276M1BxASWrapper::Write( REG_FRFLSB, ( uint8_t )( freq & 0xFF ) );
}
...
uint32_t SX1276M1BxASWrapper::Random(  )
{
    ...
    SX1276M1BxASWrapper::Write( REG_LR_IRQFLAGSMASK, RFLR_IRQFLAGS_RXTIMEOUT |
    ...
    for( i = 0; i < 32; i++ )
    {
        ...
        rnd |= ( ( uint32_t )SX1276M1BxASWrapper::Read( REG_LR_RSSIWIDEBAND ) & 0x01 ) << i;
    }
...
void SX1276M1BxASWrapper::RxChainCalibration( )
{
    ...
    // Save context
    regPaConfigInitVal = this->SX1276M1BxASWrapper::Read( 12 );

	CPU_GPIO_SetPinState( (GPIO_PIN)25 , FALSE);
	CPU_GPIO_SetPinState( (GPIO_PIN)25 , TRUE);


    initialFreq = ( double )( ( ( uint32_t )this->SX1276M1BxASWrapper::Read( REG_FRFMSB ) << 16 ) |
                              ( ( uint32_t )this->SX1276M1BxASWrapper::Read( REG_FRFMID ) << 8 ) |
                              ( ( uint32_t )this->SX1276M1BxASWrapper::Read( REG_FRFLSB ) ) ) * ( double )FREQ_STEP;

    // Cut the PA just in case, RFO output, power = -1 dBm
    this->SX1276M1BxASWrapper::Write( REG_PACONFIG, 0x00 );

    // Launch Rx chain calibration for LF band
    SX1276M1BxASWrapper::Write ( REG_IMAGECAL, ( SX1276M1BxASWrapper::Read( REG_IMAGECAL ) & RF_IMAGECAL_IMAGECAL_MASK ) | RF_IMAGECAL_IMAGECAL_START );
    while( ( SX1276M1BxASWrapper::Read( REG_IMAGECAL ) & RF_IMAGECAL_IMAGECAL_RUNNING ) == RF_IMAGECAL_IMAGECAL_RUNNING )
    {
    }
    ...
    // Restore context
    this->SX1276M1BxASWrapper::Write( REG_PACONFIG, regPaConfigInitVal );
    ...
    case MODEM_FSK:
        {
            ...
            SX1276M1BxASWrapper::Write( REG_BITRATEMSB, ( uint8_t )( datarate >> 8 ) );
            SX1276M1BxASWrapper::Write( REG_BITRATELSB, ( uint8_t )( datarate & 0xFF ) );

            SX1276M1BxASWrapper::Write( REG_RXBW, GetFskBandwidthRegValue( bandwidth ) );
            SX1276M1BxASWrapper::Write( REG_AFCBW, GetFskBandwidthRegValue( bandwidthAfc ) );

            SX1276M1BxASWrapper::Write( REG_PREAMBLEMSB, ( uint8_t )( ( preambleLen >> 8 ) & 0xFF ) );
            SX1276M1BxASWrapper::Write( REG_PREAMBLELSB, ( uint8_t )( preambleLen & 0xFF ) );

            if( fixLen == 1 )
            {
                SX1276M1BxASWrapper::Write( REG_PAYLOADLENGTH, payloadLen );
            }
            else
            {
                SX1276M1BxASWrapper::Write( REG_PAYLOADLENGTH, 0xFF ); // Set payload length to the maximum
            }

            SX1276M1BxASWrapper::Write( REG_PACKETCONFIG1,
                         ( SX1276M1BxASWrapper::Read( REG_PACKETCONFIG1 ) &
                           RF_PACKETCONFIG1_CRC_MASK &
                           RF_PACKETCONFIG1_PACKETFORMAT_MASK ) |
                           ( ( fixLen == 1 ) ? RF_PACKETCONFIG1_PACKETFORMAT_FIXED : RF_PACKETCONFIG1_PACKETFORMAT_VARIABLE ) |
                           ( crcOn << 4 ) );
            SX1276M1BxASWrapper::Write( REG_PACKETCONFIG2, ( SX1276M1BxASWrapper::Read( REG_PACKETCONFIG2 ) | RF_PACKETCONFIG2_DATAMODE_PACKET ) );
        }
        break;
    case MODEM_LORA:
        {
            ...
            SX1276M1BxASWrapper::Write( REG_LR_MODEMCONFIG1,
                         ( SX1276M1BxASWrapper::Read( REG_LR_MODEMCONFIG1 ) &
            ...
            SX1276M1BxASWrapper::Write( REG_LR_MODEMCONFIG2,
                         ( SX1276M1BxASWrapper::Read( REG_LR_MODEMCONFIG2 ) &
            ...
            SX1276M1BxASWrapper::Write( REG_LR_MODEMCONFIG3,
                         ( SX1276M1BxASWrapper::Read( REG_LR_MODEMCONFIG3 ) &
                           RFLR_MODEMCONFIG3_LOWDATARATEOPTIMIZE_MASK ) |
                           ( this->settings.LoRa.LowDatarateOptimize << 3 ) );

            SX1276M1BxASWrapper::Write( REG_LR_SYMBTIMEOUTLSB, ( uint8_t )( symbTimeout & 0xFF ) );

            SX1276M1BxASWrapper::Write( REG_LR_PREAMBLEMSB, ( uint8_t )( ( preambleLen >> 8 ) & 0xFF ) );
            SX1276M1BxASWrapper::Write( REG_LR_PREAMBLELSB, ( uint8_t )( preambleLen & 0xFF ) );

            if( fixLen == 1 )
            {
                SX1276M1BxASWrapper::Write( REG_LR_PAYLOADLENGTH, payloadLen );
            }

            if( this->settings.LoRa.FreqHopOn == true )
            {
                SX1276M1BxASWrapper::Write( REG_LR_PLLHOP, ( SX1276M1BxASWrapper::Read( REG_LR_PLLHOP ) & RFLR_PLLHOP_FASTHOP_MASK ) | RFLR_PLLHOP_FASTHOP_ON );
                SX1276M1BxASWrapper::Write( REG_LR_HOPPERIOD, this->settings.LoRa.HopPeriod );
            }

            if( ( bandwidth == 9 ) && ( this->settings.Channel > RF_MID_BAND_THRESH ) )
            {
                // ERRATA 2.1 - Sensitivity Optimization with a 500 kHz Bandwidth
                SX1276M1BxASWrapper::Write( REG_LR_TEST36, 0x02 );
                SX1276M1BxASWrapper::Write( REG_LR_TEST3A, 0x64 );
            }
            else if( bandwidth == 9 )
            {
                // ERRATA 2.1 - Sensitivity Optimization with a 500 kHz Bandwidth
                SX1276M1BxASWrapper::Write( REG_LR_TEST36, 0x02 );
                SX1276M1BxASWrapper::Write( REG_LR_TEST3A, 0x7F );
            }
            else
            {
                // ERRATA 2.1 - Sensitivity Optimization with a 500 kHz Bandwidth
                SX1276M1BxASWrapper::Write( REG_LR_TEST36, 0x03 );
            }

            if( datarate == 6 )
            {
                SX1276M1BxASWrapper::Write( REG_LR_DETECTOPTIMIZE,
                             ( SX1276M1BxASWrapper::Read( REG_LR_DETECTOPTIMIZE ) &
                               RFLR_DETECTIONOPTIMIZE_MASK ) |
                               RFLR_DETECTIONOPTIMIZE_SF6 );
                SX1276M1BxASWrapper::Write( REG_LR_DETECTIONTHRESHOLD,
    ...
    case MODEM_FSK:
        {
            ...
            SX1276M1BxASWrapper::Write( REG_FDEVMSB, ( uint8_t )( fdev >> 8 ) );
            SX1276M1BxASWrapper::Write( REG_FDEVLSB, ( uint8_t )( fdev & 0xFF ) );

            datarate = ( uint16_t )( ( double )XTAL_FREQ / ( double )datarate );
            SX1276M1BxASWrapper::Write( REG_BITRATEMSB, ( uint8_t )( datarate >> 8 ) );
            SX1276M1BxASWrapper::Write( REG_BITRATELSB, ( uint8_t )( datarate & 0xFF ) );

            SX1276M1BxASWrapper::Write( REG_PREAMBLEMSB, ( preambleLen >> 8 ) & 0x00FF );
            SX1276M1BxASWrapper::Write( REG_PREAMBLELSB, preambleLen & 0xFF );
    ...
    case MODEM_LORA:
        {
            ...
            SX1276M1BxASWrapper::Write( REG_LR_PREAMBLEMSB, ( preambleLen >> 8 ) & 0x00FF );
            SX1276M1BxASWrapper::Write( REG_LR_PREAMBLELSB, preambleLen & 0xFF );
    ...
    case MODEM_FSK:
        {
            airTime = rint( ( 8 * ( this->settings.Fsk.PreambleLen +
                                     ( ( SX1276M1BxASWrapper::Read( REG_SYNCCONFIG ) & ~RF_SYNCCONFIG_SYNCSIZE_MASK ) + 1 ) +
                                     ( ( this->settings.Fsk.FixLen == 0x01 ) ? 0.0 : 1.0 ) +
                                     ( ( ( SX1276M1BxASWrapper::Read( REG_PACKETCONFIG1 ) & ~RF_PACKETCONFIG1_ADDRSFILTERING_MASK ) != 0x00 ) ? 1.0 : 0 ) +
                                     pktLen +
    ...
    case MODEM_FSK:
        {
            ...
            {
                SX1276M1BxASWrapper::Write( REG_PAYLOADLENGTH, size );
            }
            ...
            // SX1276M1BxASWrapper::Write payload buffer
    ...
    case MODEM_LORA:
        {
            ...
            {
                SX1276M1BxASWrapper::Write( REG_LR_INVERTIQ, ( ( SX1276M1BxASWrapper::Read( REG_LR_INVERTIQ ) & RFLR_INVERTIQ_TX_MASK & RFLR_INVERTIQ_RX_MASK ) | RFLR_INVERTIQ_RX_OFF | RFLR_INVERTIQ_TX_ON ) );
                SX1276M1BxASWrapper::Write( REG_LR_INVERTIQ2, RFLR_INVERTIQ2_ON );
            }
            else
            {
                SX1276M1BxASWrapper::Write( REG_LR_INVERTIQ, ( ( SX1276M1BxASWrapper::Read( REG_LR_INVERTIQ ) & RFLR_INVERTIQ_TX_MASK & RFLR_INVERTIQ_RX_MASK ) | RFLR_INVERTIQ_RX_OFF | RFLR_INVERTIQ_TX_OFF ) );
                SX1276M1BxASWrapper::Write( REG_LR_INVERTIQ2, RFLR_INVERTIQ2_OFF );
            }

            this->settings.LoRaPacketHandler.Size = size;

            // Initializes the payload size
            SX1276M1BxASWrapper::Write( REG_LR_PAYLOADLENGTH, size );

            // Full buffer used for Tx
            SX1276M1BxASWrapper::Write( REG_LR_FIFOTXBASEADDR, 0 );
            SX1276M1BxASWrapper::Write( REG_LR_FIFOADDRPTR, 0 );

            // FIFO operations can not take place in Sleep mode
            if( ( SX1276M1BxASWrapper::Read( REG_OPMODE ) & ~RF_OPMODE_MASK ) == RF_OPMODE_SLEEP )
            {
                Standby( );
                wait_ms( 1 );
            }
    ...
    case MODEM_FSK:
        {
            ...
            // DIO4=Preamble
            // DIO5=ModeReady
            SX1276M1BxASWrapper::Write( REG_DIOMAPPING1, ( SX1276M1BxASWrapper::Read( REG_DIOMAPPING1 ) & RF_DIOMAPPING1_DIO0_MASK &
            ...
            SX1276M1BxASWrapper::Write( REG_DIOMAPPING2, ( SX1276M1BxASWrapper::Read( REG_DIOMAPPING2 ) & RF_DIOMAPPING2_DIO4_MASK &
                                                                            RF_DIOMAPPING2_MAP_MASK ) |
                                                                            RF_DIOMAPPING2_DIO4_11 |
                                                                            RF_DIOMAPPING2_MAP_PREAMBLEDETECT );

            this->settings.FskPacketHandler.FifoThresh = SX1276M1BxASWrapper::Read( REG_FIFOTHRESH ) & 0x3F;

            SX1276M1BxASWrapper::Write( REG_RXCONFIG, RF_RXCONFIG_AFCAUTO_ON | RF_RXCONFIG_AGCAUTO_ON | RF_RXCONFIG_RXTRIGER_PREAMBLEDETECT );
    ...
    case MODEM_LORA:
        {
            if( this->settings.LoRa.IqInverted == true )
            {
                SX1276M1BxASWrapper::Write( REG_LR_INVERTIQ, ( ( SX1276M1BxASWrapper::Read( REG_LR_INVERTIQ ) & RFLR_INVERTIQ_TX_MASK & RFLR_INVERTIQ_RX_MASK ) | RFLR_INVERTIQ_RX_ON | RFLR_INVERTIQ_TX_OFF ) );
                SX1276M1BxASWrapper::Write( REG_LR_INVERTIQ2, RFLR_INVERTIQ2_ON );
            }
            else
            {
            ...
            {
                SX1276M1BxASWrapper::Write( REG_LR_DETECTOPTIMIZE, SX1276M1BxASWrapper::Read( REG_LR_DETECTOPTIMIZE ) & 0x7F );
                SX1276M1BxASWrapper::Write( REG_LR_TEST30, 0x00 );
                switch( this->settings.LoRa.Bandwidth )
                {
                case 0: // 7.8 kHz
                    SX1276M1BxASWrapper::Write( REG_LR_TEST2F, 0x48 );
                    SetChannel(this->settings.Channel + 7.81e3 );
                    break;
                case 1: // 10.4 kHz
                    SX1276M1BxASWrapper::Write( REG_LR_TEST2F, 0x44 );
                ...
                case 5: // 41.4 kHz
                    ...
                    break;
                case 6: // 62.5 kHz
                    SX1276M1BxASWrapper::Write( REG_LR_TEST2F, 0x40 );
                    break;
                case 7: // 125 kHz
                    SX1276M1BxASWrapper::Write( REG_LR_TEST2F, 0x40 );
                    break;
            ...
            {
                SX1276M1BxASWrapper::Write( REG_LR_DETECTOPTIMIZE, SX1276M1BxASWrapper::Read( REG_LR_DETECTOPTIMIZE ) | 0x80 );
            }
            ...
            if( this->settings.LoRa.FreqHopOn == true )
            {
                ...
                SX1276M1BxASWrapper::Write( REG_DIOMAPPING1, ( SX1276M1BxASWrapper::Read( REG_DIOMAPPING1 ) & RFLR_DIOMAPPING1_DIO0_MASK & RFLR_DIOMAPPING1_DIO2_MASK  ) | RFLR_DIOMAPPING1_DIO0_00 | RFLR_DIOMAPPING1_DIO2_00 );
            }
            else
            {
                SX1276M1BxASWrapper::Write( REG_LR_IRQFLAGSMASK, //RFLR_IRQFLAGS_RXTIMEOUT |
                ...
                // DIO0=RxDone
                SX1276M1BxASWrapper::Write( REG_DIOMAPPING1, ( SX1276M1BxASWrapper::Read( REG_DIOMAPPING1 ) & RFLR_DIOMAPPING1_DIO0_MASK ) | RFLR_DIOMAPPING1_DIO0_00 );
            }
            SX1276M1BxASWrapper::Write( REG_LR_FIFORXBASEADDR, 0 );
            SX1276M1BxASWrapper::Write( REG_LR_FIFOADDRPTR, 0 );
        }
        break;
    }
    ...
    case MODEM_LORA:
        {
            if( this->settings.LoRa.FreqHopOn == true )
            {
                ...
                SX1276M1BxASWrapper::Write( REG_DIOMAPPING1, ( SX1276M1BxASWrapper::Read( REG_DIOMAPPING1 ) & RFLR_DIOMAPPING1_DIO0_MASK & RFLR_DIOMAPPING1_DIO2_MASK ) | RFLR_DIOMAPPING1_DIO0_01 | RFLR_DIOMAPPING1_DIO2_00 );
            }
            else
            {
                SX1276M1BxASWrapper::Write( REG_LR_IRQFLAGSMASK, RFLR_IRQFLAGS_RXTIMEOUT |
                ...
                // DIO0=TxDone
                SX1276M1BxASWrapper::Write( REG_DIOMAPPING1, ( SX1276M1BxASWrapper::Read( REG_DIOMAPPING1 ) & RFLR_DIOMAPPING1_DIO0_MASK ) | RFLR_DIOMAPPING1_DIO0_01 );
            }
        }
        break;
    }
    ...
    case MODEM_LORA:
        {
            ...
            // DIO3=CADDone
            SX1276M1BxASWrapper::Write( REG_DIOMAPPING1, ( SX1276M1BxASWrapper::Read( REG_DIOMAPPING1 ) & RFLR_DIOMAPPING1_DIO3_MASK ) | RFLR_DIOMAPPING1_DIO3_00 );
...
void SX1276M1BxASWrapper::SetTxContinuousWave( uint32_t freq, int8_t power, uint16_t time )
{
    ...
    SX1276M1BxASWrapper::Write( REG_PACKETCONFIG2, ( SX1276M1BxASWrapper::Read( REG_PACKETCONFIG2 ) & RF_PACKETCONFIG2_DATAMODE_MASK ) );
    // Disable radio interrupts
    SX1276M1BxASWrapper::Write( REG_DIOMAPPING1, RF_DIOMAPPING1_DIO0_11 | RF_DIOMAPPING1_DIO1_11 );
    SX1276M1BxASWrapper::Write( REG_DIOMAPPING2, RF_DIOMAPPING2_DIO4_10 | RF_DIOMAPPING2_DIO5_10 );
...
int16_t SX1276M1BxASWrapper::GetRssi( RadioModems_t modem )
{
    ...
    {
    case MODEM_FSK:
        rssi = -( SX1276M1BxASWrapper::Read( REG_RSSIVALUE ) >> 1 );
        break;
    case MODEM_LORA:
        if( this->settings.Channel > RF_MID_BAND_THRESH )
        {
            rssi = RSSI_OFFSET_HF + SX1276M1BxASWrapper::Read( REG_LR_RSSIVALUE );
        }
        else
        {
            rssi = RSSI_OFFSET_LF + SX1276M1BxASWrapper::Read( REG_LR_RSSIVALUE );
        }
        break;
    default:
        rssi = -1;
...
void SX1276M1BxASWrapper::SetOpMode( uint8_t opMode )
{
    ...
    SX1276M1BxASWrapper::Write( REG_OPMODE, ( SX1276M1BxASWrapper::Read( REG_OPMODE ) & RF_OPMODE_MASK ) | opMode );
}

void SX1276M1BxASWrapper::SetModem( RadioModems_t modem )
{
    if( ( SX1276M1BxASWrapper::Read( REG_OPMODE ) & RFLR_OPMODE_LONGRANGEMODE_ON ) != 0 )
    {
    ...
    case MODEM_FSK:
        Sleep( );
        SX1276M1BxASWrapper::Write( REG_OPMODE, ( SX1276M1BxASWrapper::Read( REG_OPMODE ) & RFLR_OPMODE_LONGRANGEMODE_MASK ) | RFLR_OPMODE_LONGRANGEMODE_OFF );

        SX1276M1BxASWrapper::Write( REG_DIOMAPPING1, 0x00 );
        SX1276M1BxASWrapper::Write( REG_DIOMAPPING2, 0x30 ); // DIO5=ModeReady
        break;
    case MODEM_LORA:
        Sleep( );
        SX1276M1BxASWrapper::Write( REG_OPMODE, ( SX1276M1BxASWrapper::Read( REG_OPMODE ) & RFLR_OPMODE_LONGRANGEMODE_MASK ) | RFLR_OPMODE_LONGRANGEMODE_ON );

        SX1276M1BxASWrapper::Write( REG_DIOMAPPING1, 0x00 );
        SX1276M1BxASWrapper::Write( REG_DIOMAPPING2, 0x00 );
        break;
    }
}
...
void SX1276M1BxASWrapper::SetMaxPayloadLength( RadioModems_t modem, uint8_t max )
{
    ...
    case MODEM_FSK:
        if( this->settings.Fsk.FixLen == false )
        {
            this->SX1276M1BxASWrapper::Write( REG_PAYLOADLENGTH, max );
        }
        break;
    case MODEM_LORA:
        this->SX1276M1BxASWrapper::Write( REG_LR_PAYLOADMAXLENGTH, max );
        break;
    }
}
...
void SX1276M1BxASWrapper::SetPublicNetwork( bool enable )
{
    ...
    if( enable == true )
    {
        ...
        SX1276M1BxASWrapper::Write( REG_LR_SYNCWORD, LORA_MAC_PUBLIC_SYNCWORD );
    }
    else
    {
        // Change LoRa modem SyncWord
        SX1276M1BxASWrapper::Write( REG_LR_SYNCWORD, LORA_MAC_PRIVATE_SYNCWORD );
    }
}
...
void SX1276M1BxASWrapper::OnTimeoutIrq(  )
{
    ...
    case RF_RX_RUNNING:
        if( this->settings.Modem == MODEM_FSK )
        {
            ...
            // Clear Irqs
            SX1276M1BxASWrapper::Write( REG_IRQFLAGS1, RF_IRQFLAGS1_RSSI |
                                        RF_IRQFLAGS1_PREAMBLEDETECT |
                                        RF_IRQFLAGS1_SYNCADDRESSMATCH );
            SX1276M1BxASWrapper::Write( REG_IRQFLAGS2, RF_IRQFLAGS2_FIFOOVERRUN );

            if( this->settings.Fsk.RxContinuous == true )
            {
                // Continuous mode restart Rx chain
                SX1276M1BxASWrapper::Write( REG_RXCONFIG, SX1276M1BxASWrapper::Read( REG_RXCONFIG ) | RF_RXCONFIG_RESTARTRXWITHOUTPLLLOCK );
                //rxTimeoutSyncWord.attach_us( mbed::callback( this, &SX1276M1BxASWrapper::OnTimeoutIrq ),
//                                             this->settings.Fsk.RxSingleTimeout * 1e3 );
    ...
    switch( this->settings.State )
    {
        case RF_RX_RUNNING:
            ...
            case MODEM_FSK:
                ...
                {
                    irqFlags = SX1276M1BxASWrapper::Read( REG_IRQFLAGS2 );
...
//                                                         this->settings.Fsk.RxSingleTimeout * 1e3 );
 SetTimeoutTimer(rxTimeoutSyncWord, this->settings.Fsk.RxSingleTimeout * 1e3 );
                ...
                // SX1276M1BxASWrapper::Read received packet size
                if( ( this->settings.FskPacketHandler.Size == 0 ) && ( this->settings.FskPacketHandler.NbBytes == 0 ) )
                {
                    ...
                    {
                        this->settings.FskPacketHandler.Size = SX1276M1BxASWrapper::Read( REG_PAYLOADLENGTH );
                    }
...
//                                                 this->settings.Fsk.RxSingleTimeout * 1e3 );
            ...
            case MODEM_LORA:
                {
                    ...
                    // Clear Irq
                    SX1276M1BxASWrapper::Write( REG_LR_IRQFLAGS, RFLR_IRQFLAGS_RXDONE );

                    irqFlags = SX1276M1BxASWrapper::Read( REG_LR_IRQFLAGS );
                    if( ( irqFlags & RFLR_IRQFLAGS_PAYLOADCRCERROR_MASK ) == RFLR_IRQFLAGS_PAYLOADCRCERROR )
                    {
                        // Clear Irq
                        SX1276M1BxASWrapper::Write( REG_LR_IRQFLAGS, RFLR_IRQFLAGS_PAYLOADCRCERROR );
 ...
 CancelTimeoutTimer(rxTimeoutTimer);
                    ...
                    this->settings.LoRaPacketHandler.SnrValue = SX1276M1BxASWrapper::Read( REG_LR_PKTSNRVALUE );
                    ...
                    int16_t rssi = SX1276M1BxASWrapper::Read( REG_LR_PKTRSSIVALUE );
                    if( snr < 0 )
                    {
                        if( this->settings.Channel > RF_MID_BAND_THRESH )
                        {
                    ...
                    this->settings.LoRaPacketHandler.Size = SX1276M1BxASWrapper::Read( REG_LR_RXNBBYTES );
 ...
 CancelTimeoutTimer(txTimeoutTimer);
            ...
            case MODEM_LORA:
                // Clear Irq
                SX1276M1BxASWrapper::Write( REG_LR_IRQFLAGS, RFLR_IRQFLAGS_TXDONE );
...
void SX1276M1BxASWrapper::OnDio1Irq(  )
{
 ...
 CancelTimeoutTimer(rxTimeoutTimer);
                // Clear Irq
                SX1276M1BxASWrapper::Write( REG_LR_IRQFLAGS, RFLR_IRQFLAGS_RXTIMEOUT );
        ...
        case RF_TX_RUNNING:
            ...
            case MODEM_FSK:
                ...
                {
                    // SX1276M1BxASWrapper::Write the last chunk of data
...
void SX1276M1BxASWrapper::OnDio2Irq(  )
{
 ...
 CancelTimeoutTimer(rxTimeoutSyncWord);
                    ...
                    this->settings.FskPacketHandler.RssiValue = -( SX1276M1BxASWrapper::Read( REG_RSSIVALUE ) >> 1 );

                    this->settings.FskPacketHandler.AfcValue = ( int32_t )( double )( ( ( uint16_t )SX1276M1BxASWrapper::Read( REG_AFCMSB ) << 8 ) |
                                                                           ( uint16_t )SX1276M1BxASWrapper::Read( REG_AFCLSB ) ) *
                                                                           ( double )FREQ_STEP;
                    this->settings.FskPacketHandler.RxGain = ( SX1276M1BxASWrapper::Read( REG_LNA ) >> 5 ) & 0x07;
                }
                break;
            case MODEM_LORA:
                if( this->settings.LoRa.FreqHopOn == true )
                {
                    // Clear Irq
                    SX1276M1BxASWrapper::Write( REG_LR_IRQFLAGS, RFLR_IRQFLAGS_FHSSCHANGEDCHANNEL );

                    if( ( this->RadioEvents != NULL ) && ( this->RadioEvents->FhssChangeChannel != NULL ) )
                    {
                        this->RadioEvents->FhssChangeChannel( ( SX1276M1BxASWrapper::Read( REG_LR_HOPCHANNEL ) & RFLR_HOPCHANNEL_CHANNEL_MASK ) );
                    }
                }
                break;
            default:
...
void SX1276M1BxASWrapper::OnDio3Irq(  )
{
    ...
    case MODEM_LORA:
    	if( this->settings.State == RF_TX_RUNNING){ //BK: Adding interrupt for packet detected
    		if( ( SX1276M1BxASWrapper::Read( REG_LR_IRQFLAGS ) & RFLR_IRQFLAGS_VALIDHEADER ) == RFLR_IRQFLAGS_VALIDHEADER ){ //BK:Adding callback for
    			// Clear Irq
    		        	 SX1276M1BxASWrapper::Write( REG_LR_IRQFLAGS, RFLR_IRQFLAGS_VALIDHEADER | RFLR_IRQFLAGS_VALIDHEADER );
        ...
        if( ( SX1276M1BxASWrapper::Read( REG_LR_IRQFLAGS ) & RFLR_IRQFLAGS_CADDETECTED ) == RFLR_IRQFLAGS_CADDETECTED )
        {
            // Clear Irq
            SX1276M1BxASWrapper::Write( REG_LR_IRQFLAGS, RFLR_IRQFLAGS_CADDETECTED | RFLR_IRQFLAGS_CADDONE );
        ...
        {
            // Clear Irq
            SX1276M1BxASWrapper::Write( REG_LR_IRQFLAGS, RFLR_IRQFLAGS_CADDONE );
            if( ( this->RadioEvents != NULL ) && ( this->RadioEvents->CadDone != NULL ) )
            {
