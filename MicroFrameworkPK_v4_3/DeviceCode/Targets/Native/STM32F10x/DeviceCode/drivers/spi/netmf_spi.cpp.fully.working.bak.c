...
TODO - 
		...
		Disable NSS (both HW and SW) and use the MF defined pin		
	...
	*/
    ...
    {		
		...
		case 1:
			...
			if (WriteCount != 0)
			{
			    SPI_I2S_SendData(SPIy, Write16[0]);
			}
			else
			{			
				SPI_I2S_SendData(SPIy, 0);
			}


			//wait till we are ready to recieve		
			while (SPI_I2S_GetFlagStatus(SPIy, SPI_I2S_FLAG_RXNE) == RESET);
			Data16 = SPI_I2S_ReceiveData(SPIy);		
			break;
    ...
    {		
		...
		switch(Transaction.SPI_mod)
		{
			case 0:
				...
				{						
					SPI_I2S_SendData(SPIx, (UINT16)0);				
				}
			...
			case 1:
				...
				if(WriteCount != 0)
				{
					SPI_I2S_SendData(SPIy, (UINT16)Write8[0]); //TODO:	Need to check on the cast, just casting as of now
				}
				else
				{						
					SPI_I2S_SendData(SPIy, (UINT16)0);				
				}

				// wait for data reception
				while (SPI_I2S_GetFlagStatus(SPIy, SPI_I2S_FLAG_RXNE) == RESET);        
				Data8 = (UINT8) SPI_I2S_ReceiveData(SPIy);	
...
void SPI_StructInit(const SPI_CONFIGURATION& Configuration)
{
  ...
  //FIXME: 
  //Check the SSOE bit value and make sure this is the right state
