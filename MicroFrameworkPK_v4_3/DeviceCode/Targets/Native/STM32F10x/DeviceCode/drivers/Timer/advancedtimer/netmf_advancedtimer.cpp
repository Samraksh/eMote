...
BOOL STM32F10x_AdvancedTimer::AddTicks(UINT64 ticksToAdd)
{
	...
	UINT64 ticksToAdd_TIM1 = ticksToAdd & 0xffff;
	UINT64 ticksToAdd_TIM2 = (ticksToAdd & 0xffff0000) >> 16;
	UINT64 ticksToAdd_UPPER32 = ((ticksToAdd & 0xffffffff00000000) >> 32);

	GLOBAL_LOCK(irq);
	UINT64 TIM1_new = TIM1->CNT + ticksToAdd_TIM1;
	UINT64 TIM2_new = TIM2->CNT + ticksToAdd_TIM2 + (TIM1_new >> 16);
	UINT64 systemTime_new = (g_STM32F10x_AdvancedTimer.m_systemTime >> 32) + ticksToAdd_UPPER32 + (TIM2_new >> 16);
	TIM1_new = TIM1_new & 0xffff;
	TIM2_new = TIM2_new & 0xffff;
	TIM1->CNT = TIM1_new;
	TIM2->CNT = TIM2_new;
	//hal_printf("%llx %llx %llx %llx %llx\r\n", g_STM32F10x_AdvancedTimer.m_systemTime, (systemTime_new<<32), systemTime_new, (TIM2_new<<16) , TIM1_new);
	g_STM32F10x_AdvancedTimer.m_systemTime = (systemTime_new<<32) + (TIM2_new<<16) + TIM1_new;
	//hal_printf("%llx %llx %llx %llx %llx\r\n", g_STM32F10x_AdvancedTimer.m_systemTime, (systemTime_new<<32), systemTime_new, (TIM2_new<<16) , TIM1_new);

	// it should only take a few thousand ticks to calculate everything...a rollover could happen though so we just do it again
	if ((g_STM32F10x_AdvancedTimer.m_systemTime - (originalTime + ticksToAdd)) > 3000){
		originalTime =Get64Counter();
		TIM1_new = TIM1->CNT + ticksToAdd_TIM1;
		TIM2_new = TIM2->CNT + ticksToAdd_TIM2 + (TIM1_new >> 16);
		systemTime_new = (g_STM32F10x_AdvancedTimer.m_systemTime >> 32) + ticksToAdd_UPPER32 + (TIM2_new >> 16);
		TIM1_new = TIM1_new & 0xffff;
		TIM2_new = TIM2_new & 0xffff;
		TIM1->CNT = TIM1_new;
		TIM2->CNT = TIM2_new;
	...
	//hal_printf("%llx %llx %llx %llx %llx %llx %llx\r\n",g_STM32F10x_AdvancedTimer.m_systemTime, Get64Counter(), (originalTime ), (g_STM32F10x_AdvancedTimer.m_systemTime - (originalTime + ticksToAdd)), (UINT64)(systemTime_new<<32), (UINT64)(TIM2->CNT<<16), (UINT64)TIM1->CNT);
	//hal_printf("%llx %llx %llx %llx %llx %llx\r\n", g_STM32F10x_AdvancedTimer.m_systemTime, (UINT64)(systemTime_new<<32), systemTime_new, (UINT64)(systemTime_new<<32) , (UINT64)(TIM2->CNT<<16) , (UINT64)TIM1->CNT);
