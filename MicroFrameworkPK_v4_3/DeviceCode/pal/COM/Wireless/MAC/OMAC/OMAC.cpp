...
DeviceStatus OMACType::SetOMACParametersBasedOnRadioName(UINT8 radioName){
	...
	switch(radioName){
		...
		case RF231RADIO:
		{
			...
			MAX_PACKET_TX_DURATION_MICRO = 5*MICSECINMILISEC;
			ACK_RX_MAX_DURATION_MICRO = 20*MICSECINMILISEC;
			DISCO_BEACON_TX_MAX_DURATION_MICRO = 1.2*MICSECINMILISEC;
			DISCO_PACKET_TX_TIME_MICRO = 1*MICSECINMILISEC;
			DISCO_SLOT_PERIOD_MICRO = 8*MICSECINMILISEC;
			HIGH_DISCO_PERIOD_IN_SLOTS = 9000;
			if(CPU_Radio_GetRadioAckType() == SOFTWARE_ACK){
				DELAY_IN_RECEIVING_HW_ACK = 0*MICSECINMILISEC;
				DELAY_IN_RECEIVING_SW_ACK = 0.6*MICSECINMILISEC;
				RETRANS_DELAY_DUE_TO_MISSING_HW_ACK = 0*MICSECINMILISEC;
				RETRANS_DELAY_DUE_TO_MISSING_SW_ACK = 0.2*MICSECINMILISEC;
				ACK_TX_MAX_DURATION_MICRO = 4*MICSECINMILISEC;
			}
			else if(CPU_Radio_GetRadioAckType() == HARDWARE_ACK){
				DELAY_IN_RECEIVING_HW_ACK = 0.4*MICSECINMILISEC;
				DELAY_IN_RECEIVING_SW_ACK = 0*MICSECINMILISEC;
				RETRANS_DELAY_DUE_TO_MISSING_HW_ACK = 0*MICSECINMILISEC;
				RETRANS_DELAY_DUE_TO_MISSING_SW_ACK = 0*MICSECINMILISEC;
				ACK_TX_MAX_DURATION_MICRO = 0*MICSECINMILISEC;
			}
			else{
				ASSERT_SP(0);
			}
		...
		{
			RETRY_RANDOM_BACKOFF_DELAY_MICRO = (RANDOM_BACKOFF_COUNT_MAX*DELAY_DUE_TO_CCA_MICRO);
			MAX_PACKET_TX_DURATION_MICRO = 27.6*MICSECINMILISEC;
			ACK_RX_MAX_DURATION_MICRO = 8*MICSECINMILISEC;
			DISCO_BEACON_TX_MAX_DURATION_MICRO = 10*MICSECINMILISEC;
			DISCO_PACKET_TX_TIME_MICRO = 10*MICSECINMILISEC;
			DISCO_SLOT_PERIOD_MICRO = 8*MICSECINMILISEC;
			HIGH_DISCO_PERIOD_IN_SLOTS = 9000;
			if(CPU_Radio_GetRadioAckType() == SOFTWARE_ACK){
				DELAY_IN_RECEIVING_HW_ACK = 0*MICSECINMILISEC;
				...
				RETRANS_DELAY_DUE_TO_MISSING_HW_ACK = 0*MICSECINMILISEC;
				RETRANS_DELAY_DUE_TO_MISSING_SW_ACK = 0*MICSECINMILISEC;
				ACK_TX_MAX_DURATION_MICRO = 8*MICSECINMILISEC;;
			}
			else if(CPU_Radio_GetRadioAckType() == HARDWARE_ACK){
				ASSERT_SP(0);
			}
			...
			DELAY_IN_RECEIVING_ACK = 3.6*MICSECINMILISEC;
			RETRANS_DELAY_DUE_TO_MISSING_ACK = 12.8*MICSECINMILISEC;
