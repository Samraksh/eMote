...
void HALTimerCallback(void *arg)
{
	...
	VirtualTimer* topTimer = gVirtualTimerManagerObject.timerQueue.PeekTop();
	...
	UINT64 timeElapsed = HAL_Time_CurrentTicks() - gVirtualTimerManagerObject.get_m_lastInterruptFireTime();
	gVirtualTimerManagerObject.set_m_lastInterruptFireTime(HAL_Time_CurrentTicks());

	for(UINT16 i = 0; i < gVirtualTimerManagerObject.m_current_timer_id_; i++)
	{
		//gVirtualTimerManagerObject.m_timer[i].set_m_ticksTillExpire(gVirtualTimerManagerObject.m_timer[i].get_m_ticksTillExpire() - g_STM32F10x_AdvancedTimer.GetCounter());
		gVirtualTimerManagerObject.m_timer[i].set_m_ticksTillExpire(gVirtualTimerManagerObject.m_timer[i].get_m_ticksTillExpire() - (INT64) timeElapsed);
	}

	// Callback all timers that need to be served at this point
	while(topTimer->get_m_ticksTillExpire() <= 0)
	{
		topTimer = gVirtualTimerManagerObject.timerQueue.ExtractTop();
		...
		if(topTimer->get_m_is_running())
			gVirtualTimerManagerObject.timerQueue.Insert(topTimer);

		topTimer = gVirtualTimerManagerObject.timerQueue.PeekTop();
	}
	...
	if(topTimer)
	{
		...
		CPU_TIMER_SetCompare(*((UINT32 *)arg), topTimer->get_m_ticksTillExpire());
	}
}
...
{
	// Start Up Timer
	// Created a new PAL-HAL connection to ensure that all code is platform independent.
	// VIRTUALTIMER is a macro defined under platform_selector.h, defining this anywhere else breaks platform abstraction
	UINT32 Timer = 1;
	BOOL FreeRunning = false;
	UINT32 ClkSource = 0;
	UINT32 Prescaler = 0;

	if(!CPU_TIMER_Initialize(Timer, FreeRunning, ClkSource, Prescaler, HALTimerCallback, NULL))
		return DS_Fail;
...
BOOL VirtualTimerManager::StartTimer(UINT8 timer_id)
{
	...
	for(UINT16 i = 0; i < m_current_timer_id_; i++)
	{
		if(m_timer[i].get_m_timer_id() == timer_id)
		{
			...
			VirtualTimer *nextTimer = timerQueue.PeekTop();
			...
			if(nextTimer->get_m_timer_id() == timer_id)
			{
				...
				CPU_TIMER_SetCompare(currentTime, nextTimer->get_m_ticksTillExpire());
			}
...
BOOL VirtualTimerManager::CreateTimer(UINT8 timer_id, UINT32 start_time, UINT32 dtime, BOOL is_one_shot, BOOL _isreserved, TIMER_CALLBACK_FPN callback)
{
	...
	if(m_active_timer == NULL)
	{
		...
		CPU_TIMER_SetCompare(0, nextTimer->get_m_ticksTillExpire());
...
BOOL VirtualTimerManager::StopTimer(UINT8 timer_id)
{
	...
	UINT8* active_timer = gVirtualTimerManagerObject.GetActiveTimerList();

	for(int i = 0; i < gVirtualTimerManagerObject.get_m_number_of_active_timers(); i++)
	{
...
void HALTimerCallback(void *arg)
{
	...
	HALTimer* timers = gVirtualTimerManagerObject.get_m_timer();
	UINT8* active_timer = gVirtualTimerManagerObject.GetActiveTimerList();
	...
	for(int i = 0; i < gVirtualTimerManagerObject.get_m_number_of_active_timers(); i++)
	{
		...
		{
			if(timers[active_timer[i]].get_m_ticksTillExpire() > (gVirtualTimerManagerObject.get_timer_resolution()+ (250 + 20 * i)))
			{
				...
				timers[active_timer[i]].set_m_ticksTillExpire(timers[active_timer[i]].get_m_ticksTillExpire() - (gVirtualTimerManagerObject.get_timer_resolution() + (250 + 20 * i)));
				//timers[active_timer[i]].set_m_ticksTillExpire(timers[active_timer[i]].get_m_ticksTillExpire() - (gVirtualTimerManagerObject.get_timer_resolution() + skew));
			}
			else
			{
				// Make the callback
	...
	if(gVirtualTimerManagerObject.get_highResolutionMode())
	{
		CPU_TIMER_SetCompare(HALTIMER, (UINT16)gVirtualTimerManagerObject.get_timer_resolution());
	}
}

#if 0
void VirtualTimerManager::HALTimerCallback(void *arg)
{
