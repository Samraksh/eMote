...
bool CLR_Messaging::Phy_TransmitMessage( void* state, const WP_Message* msg )
{
	...
	//LCD_DisplayStringLine(Line0, (uint8_t *)"Phy_TransmitMessage");
...
bool CLR_Messaging::TransmitMessage( const WP_Message* msg, bool fQueue )
{
	...
	//LCD_DisplayStringLine(Line2, (uint8_t *)"TransmitMessage");
    ...
    {
    	//LCD_DisplayStringLine(Line6, (uint8_t *)"Another IF");
        if(DebuggerPort_Write( m_port, (char*)msg->m_payload, msg->m_header.m_size ) != msg->m_header.m_size) return false;
    }

	//LCD_DisplayStringLine(Line7, (uint8_t *)"Before Dbg_Flush");
		
    DebuggerPort_Flush( m_port );

	//LCD_DisplayStringLine(Line9, (uint8_t *)"After Dbg_Flush");
...
{
    NATIVE_PROFILE_CLR_MESSAGING();
	//LCD_DisplayStringLine(Line1, (uint8_t *)"NUM_DEBUGGERS == 1");
