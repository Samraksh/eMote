...
namespace Interop.SimpleService
{
    ...
    class AttachmentService : DpwsHostedService
    {
        ...
        public byte[] OneWayAttachment(WsWsaHeader header, XmlReader reader)
        {
            ...
            if ((bodyPart = BodyParts[cid]) == null)
        ...
        public byte[] TwoWayAttachmentRequest(WsWsaHeader header, XmlReader reader)
        {
            // Make sure we have MTOM body parts
            if (this.BodyParts == null)
                throw new WsFaultException(header, WsFaultType.XmlException, "MTOM message has no body parts.");
        ...
        public byte[] TwoWayAttachmentResp(WsWsaHeader header)
        {
