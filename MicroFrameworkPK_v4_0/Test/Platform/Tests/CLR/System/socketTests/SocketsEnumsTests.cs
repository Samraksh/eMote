             ...
             SocketOptionName optionName, Object data, bool isSupported)
        {
            ...
            catch (Exception e)
            {
                ...
                if (e.GetType() == Type.GetType("System.Net.Sockets.SocketException"))
                {
                    ...
                    if(!isSupported && (((SocketException)e).ErrorCode == 10042 || ((SocketException)e).ErrorCode == 10022|| ((SocketException)e).ErrorCode == 10049))
                    {
                        Log.Comment("Non-supported option graceful fail");
                        return true;
                    }
                    else if(isSupported && ((SocketException)e).ErrorCode == 10042)
                    {
                        Log.Comment(
                            "Supported option graceful fail, this option needs implementation");
                    }
                    else if (!isSupported && (((SocketException)e).ErrorCode != 10042 || ((SocketException)e).ErrorCode != 10022|| ((SocketException)e).ErrorCode != 10049))
                    {
