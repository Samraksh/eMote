...
namespace STCDemo
{
    ...
    public class STCDemo
    {
        ...
        public static class StateMachine
        {
            ...
            public static int GETNOISE = 1;
            public static int GOTNOISE = 2;
            public static int DLNOISE = 3;
            public static int NOISEDONE = 4;
            public static int RUNDETECT = 5;
            public static int DETECTED = 6;
            public static int DLDATA = 7;
		...
		public static class mySensor
		{
			...
			public static void readPair(ushort[] result)
			{
				ADC.getData(result, 0, 2);
			}
		}
        ...
        public static class mySensor
        {
            // Constants defined according to the MAX1363 datasheet
            const byte adc_config = 0x1b;     // 0, 00 (scan mode), 11 (enable readback), 01 (scan from ch 00 to ch 01), 1 (single-ended)
            const byte adc_setup = 0x82;      // 1, 00 (ref = VDD), 0 (int ref off), 0 (int clock), 0 (unipolar), 10

            public static byte[] wrByte = { adc_config, adc_setup };
            public static byte[] readByte = new byte[4];

            public static I2CDevice myi2c = new I2CDevice(new I2CDevice.Configuration(0x34, 400));
            public static I2CDevice.I2CWriteTransaction command = myi2c.CreateWriteTransaction(wrByte);
            public static I2CDevice.I2CTransaction[] xAction = { command };

            public static I2CDevice.I2CReadTransaction data = myi2c.CreateReadTransaction(readByte);
            public static I2CDevice.I2CTransaction[] xAction_Read = { data };
            ...
            // Returns ADC conversions from Channels 0 and 1 
            public static void readPair(ushort[] result)
            {
                myi2c.Execute(xAction, 20);
                myi2c.Execute(xAction_Read, 20);

                result[0] = (ushort)(((readByte[0] & 0x0f) << 8) + readByte[1]);
                result[1] = (ushort)(((readByte[2] & 0x0f) << 8) + readByte[3]);
            }
        }
		*/
		/*
		public class mySensorData
		{
			...
			public static int dataBufWidth = 3;
			
			public ushort[] Idata = new ushort[rate * dataBufWidth];
            public ushort[] Qdata = new ushort[rate * dataBufWidth];

			public static int meanI = 0;
            public static int meanQ = 0;

            public static int logIndex, startIndex, stopIndex, noiseIndex;

			public mySensorData()
			{
			    for (int i = 0; i < rate * dataBufWidth; i++)
                {
                    Idata[i] = Qdata[i] = 0;
            
                }
				logIndex = startIndex = stopIndex = 0;
			}
		}
		*/
        ...
        public static class SensorData
        {
            ...
            //public static ushort[] Inoise = new ushort[rate * noiseBufWidth];
            //public static ushort[] Qnoise = new ushort[rate * noiseBufWidth];

			//public static ushort[] Inoise = new ushort[100];
            //public static ushort[] Qnoise = new ushort[100];
            ...
            public static void initNoise()
            {
				...
				for (int i = 0; i < rate * noiseBufWidth; i++)
                {
                    Idata[i] = 0;
					Qdata[i] = 0;					
                }
        ...
        public static class RadioParams
        {
            ...
            public static ushort panID = 0x0022;
            public static ushort nodeID;
            public static ushort destID = 0xffff;
            public static ushort rcvParam = 0xffff;

            // Figure out node IDs
            // STC1: SID = 60, SKU = 1832, UID = 133
            // STC2: SID = 3179, SKU = 2081, UID = 3252
            // STC3: SID = 26, SKU = 1832, UID = 99
            // STC4: SID = 292, SKU = 1832, UID = 365
            ...
            public static void init()
            {
				...
				nodeID = 0x02;
			/*
                if (UniqueID.SerialID == 60)
                    nodeID = 0x01;
                else if (UniqueID.SerialID == 3179)
                    nodeID = 0x02;
                else if (UniqueID.SerialID == 26)
                    nodeID = 0x03;
                else if (UniqueID.SerialID == 292)
                    nodeID = 0x04;
			*/
				MessageLayer.Init();
                //Debug.Print("My node id is " + nodeID);
            }
        }
        ...
        {
            // temp variables - declared static to avoid GC
            public static ushort[] sample = new ushort[2];
            public static long nextTime, tickDiff;
            public static int detectStarted, detectFinished;
            public static int lastDetected, sampCtr;
            
            public static Comp currSample;

            public static int tempPhase, lastPhase, diffPhase, runningPhase;
            public static int phaseIndex;

            public static int[] phaseArray = new int[SensorData.rate * 2];
            
            public static void init()
            {
                detectStarted = detectFinished = 0;
                tempPhase = lastPhase = diffPhase = runningPhase = 0;
                lastDetected = sampCtr = 0;
                tickDiff = (long)30000; // 30000x100ns = 3ms
				
                for (phaseIndex = 0; phaseIndex < SensorData.rate * 2; phaseIndex++)
                    phaseArray[phaseIndex] = 0;
		...
		{
			public ushort[] sample = new ushort[2];
			public long nextTime, tickDiff;
			public int detectStarted, detectFinished;
			public int lastDetected, sampCtr;
			
			public Comp currSample;
			
			public int tempPhase, lastPhase, diffPhase, runningPhase;
            public int phaseIndex;
			
			public int[] phaseArray = new int[SensorData.rate * 2];
		...
		*/
        ...
        public static class MofNParams
        {
            ...
            public static int[] Buff = new int[2];
            public static int state = 0;
            public static int prevstate = 0;
            private static int i;
            public static int End;

            public static int startOffset, stopOffset;
        ...
        public static class Phase
        {
            ...
            public static int unwrap(Comp a)
            {
                newPhase = 0;
        ...
        public static void collectNoise()
        {
            ...
            {
                mySensor.readPair(NoiseState.sample);
                myIO.toggle();
        ...
        public static void sendNoise()
        {
			...
			uint currIndex = 0;
            int numNoisePkts = SensorData.rate * SensorData.noiseBufWidth / 5;
            int packetIndex;

            for (uint i = 0; i < numNoisePkts; i++)
            {
                // construct tinyos headers so TelosB base can receive it
				// Nived : Commenting out the TinyOS header
				
				/*
                packet[0] = (byte)63;
                packet[1] = (byte)239;
				*/
                packet[0] = (byte)((RadioParams.nodeID >> 8) & 0xff);
                packet[1] = (byte)(RadioParams.nodeID & 0xff);
				packet[2] = (byte)(i >> 8);
                packet[3] = (byte)(i);
                packet[4] = (byte)(00);
                packet[5] = (byte)(12);
                ...
                packetIndex = 6;
                // pack 20 samples to a packet
				//FileSystem.Read(packet,i,20);
				
                for (uint j = 0; j < 5; j++)
                {
                    //uint currIndex = i * 5 + j;
					
                    packet[packetIndex++] = (byte)((SensorData.Idata[currIndex] >> 8) & 0xff);
                    packet[packetIndex++] = (byte)((SensorData.Idata[currIndex]) & 0xff);
                    packet[packetIndex++] = (byte)((SensorData.Qdata[currIndex] >> 8) & 0xff);
                    packet[packetIndex++] = (byte)((SensorData.Qdata[currIndex]) & 0xff);
					currIndex++;
					
                }
				
                //myRadio.Send(RadioParams.panID, RadioParams.destID, packet);
				
                MessageLayer.Send(0,0xffff,packet,26);
				//rDAC.Init();
				Thread.Sleep(20);
            }

            // send a special completion packet so base station knows to move on
            /*
			packet[0] = (byte)63;
            packet[1] = (byte)239;
			*/
        ...
        public static void runDetectorMofN()
        {
            ...
            dState.init();
			//DetectorState dState = new DetectorState();
			if(secondTime)
			{
					rDAC.Init();
					//secondTime = false;
            ...
            dState.nextTime = DateTime.Now.Ticks;

            while (dState.detectFinished == 0)
            {
                mySensor.readPair(dState.sample);
                myIO.toggle();
                ...
                // log all data because it may be evidence for some near-future detection
                SensorData.Idata[SensorData.logIndex] = dState.sample[0];
                SensorData.Qdata[SensorData.logIndex] = dState.sample[1];

                SensorData.logIndex++;
                if (SensorData.logIndex == (SensorData.rate * SensorData.dataBufWidth))
                    SensorData.logIndex = 0;

                // remove DC from sample
                dState.currSample.I = (int)(dState.sample[0]) - SensorData.meanI;
                dState.currSample.Q = (int)(dState.sample[1]) - SensorData.meanQ;

                // get unwrapped phase
				
                dState.tempPhase = Phase.unwrap(dState.currSample) / 4096;
                
                // we dont track phase change over continuous windows, rather we track phase change
                // over 1s snippets and use M out of N logic over whether there is motion in each snippet

                // reset at the start of each snippet & track max phase and min phase over each snippet
                if (MofNParams.snippetIndex == 0)
                    MofNParams.snippetMin = MofNParams.snippetMax = dState.tempPhase;
                else if (dState.tempPhase > MofNParams.snippetMax)
                    MofNParams.snippetMax = dState.tempPhase;
                else if (dState.tempPhase < MofNParams.snippetMin)
                    MofNParams.snippetMin = dState.tempPhase;
				/*
				if (MofNParams.snippetIndex == 0)
                    CumulativeCuts.reset();
                CumulativeCuts.update(DetectorState.currSample);
				*/
                MofNParams.snippetIndex++;

                if (MofNParams.snippetIndex == SensorData.rate) // one second snippet
                {
                    // if phase change exceeds thresh, this snippet has a 'hit', else miss
                    // if there are M hits in last N snippets, we have a detect
                   // if (MofNParams.snippetMax - MofNParams.snippetMin > MofNParams.Thresh)
				  //  if (System.Math.Abs(CumulativeCuts.cumCuts) > 5)
				   if (MofNParams.snippetMax - MofNParams.snippetMin > 75)
                        MofNParams.update(MofNParams.snippetNum, 1);
                    else
                        MofNParams.update(MofNParams.snippetNum, 0);

                    MofNParams.snippetNum++;
                    MofNParams.snippetIndex = 0;

                    // if a new detect starts now, there must be some motion in past
                    if (MofNParams.prevstate == 0 && MofNParams.state == 1)
                    {
                        SensorData.startIndex = calcMofNStartIndex(); // 3 seconds into the past
                    }

                    // if an ongoing detect ends, it must have ended some time back
                    else if (MofNParams.state == 0 && MofNParams.prevstate == 1)
                    {
                ...
                //myIO.toggle();
                while (DateTime.Now.Ticks <= dState.nextTime) ;
            }
        }
		...
		*/
		...
		public static void runDetectorMofN()
        {
            ...
            //SensorData.initData();
			mySensorData ms = new mySensorData();
            //Phase.init();
			CumulativeCuts.init();
            ...
            ds.nextTime = DateTime.Now.Ticks;

            while (ds.detectFinished == 0)
            {
                mySensor.readPair(ds.sample);
                //myIO.toggle();

                // fix next sampling instant
                while (ds.nextTime < DateTime.Now.Ticks)
                    ds.nextTime += ds.tickDiff;

                ds.sampCtr++;

                // log all data because it may be evidence for some near-future detection
                ms.Idata[mySensorData.logIndex] = ds.sample[0];
                ms.Qdata[mySensorData.logIndex] = ds.sample[1];

                mySensorData.logIndex++;
                if (mySensorData.logIndex == (mySensorData.rate * mySensorData.dataBufWidth))
                    mySensorData.logIndex = 0;

                // remove DC from sample
                ds.currSample.I = (int)(ds.sample[0]) - mySensorData.meanI;
                ds.currSample.Q = (int)(ds.sample[1]) - mySensorData.meanQ;

                // get unwrapped phase
				/*
                ds.tempPhase = Phase.unwrap(ds.currSample) / 4096;
                ...
                // reset at the start of each snippet & track max phase and min phase over each snippet
                if (MofNParams.snippetIndex == 0)
                    MofNParams.snippetMin = MofNParams.snippetMax = ds.tempPhase;
                else if (ds.tempPhase > MofNParams.snippetMax)
                    MofNParams.snippetMax = ds.tempPhase;
                else if (ds.tempPhase < MofNParams.snippetMin)
                    MofNParams.snippetMin = ds.tempPhase;
				
				if (MofNParams.snippetIndex == 0)
                    CumulativeCuts.reset();
                CumulativeCuts.update(ds.currSample);
				
                MofNParams.snippetIndex++;

                if (MofNParams.snippetIndex == mySensorData.rate) // one second snippet
                {
                    // if phase change exceeds thresh, this snippet has a 'hit', else miss
                    // if there are M hits in last N snippets, we have a detect
                    if (MofNParams.snippetMax - MofNParams.snippetMin > MofNParams.Thresh)
                    ...
                    if (MofNParams.prevstate == 0 && MofNParams.state == 1)
                    {
                        mySensorData.startIndex = calcMofNStartIndex(); // 3 seconds into the past
                    }
                    ...
                    {
                        ds.detectFinished = 1;
                        mySensorData.stopIndex = calcMofNStopIndex();
                    }
                }

                //myIO.toggle();
                while (DateTime.Now.Ticks <= ds.nextTime) ;
            }
        }
		*/

        public static int calcDataSamples()
        {
            // check if buffer has rollover
            if (SensorData.stopIndex >= SensorData.startIndex)
                return (SensorData.stopIndex - SensorData.startIndex + 1);
            else
                return (SensorData.rate*SensorData.dataBufWidth + 1 - (SensorData.startIndex - SensorData.stopIndex));
        }

        public static int calcStartIndex()
        {
            if (SensorData.logIndex >= 3 * SensorData.rate)
                return (SensorData.logIndex - 3 * SensorData.rate);
            else
                return (SensorData.rate * SensorData.dataBufWidth - (3 * SensorData.rate - SensorData.logIndex));
        }

        public static int calcStopIndex()
        {
            if (SensorData.logIndex >= 6 * SensorData.rate)
                return (SensorData.logIndex - 6 * SensorData.rate);
            else
                return (SensorData.rate * SensorData.dataBufWidth - (6 * SensorData.rate - SensorData.logIndex));
        }

        public static int calcMofNStartIndex()
        {
            // go back 3 seconds - 3 seconds is an empirically derived measure
            MofNParams.startOffset = MofNParams.snippetNum - MofNParams.Buff[MofNParams.End] + 3;

            if (SensorData.logIndex >= MofNParams.startOffset * SensorData.rate)
                return (SensorData.logIndex - MofNParams.startOffset * SensorData.rate);
            else
                return (SensorData.rate * SensorData.dataBufWidth - (MofNParams.startOffset * SensorData.rate - SensorData.logIndex));
        }

        public static int calcMofNStopIndex()
        {
            // ideally we should be returning some truncated version, but based on Matlab analysis
            // the dying tail can contain some useful info, so we are being conservative here
            // we can always throw out data in Matlab
            return SensorData.logIndex;
        }

        public static void sendDetectedMessage()
        {
            //SerialDump.print("Sending detection message");
        ...
        public static void sendData()
        {
            ...
            // calculate total num pkts to send & pack 20 samples to a pkt
			//rDAC.setValue(SensorData.startIndex);
			//rDAC.setValue(SensorData.stopIndex);
			//TimeSync.setBeaconRate(SensorData.startIndex);
			//rDAC.Init();
			//TimeSync.setBeaconRate(SensorData.stopIndex);
			
            numDataSamples = calcDataSamples();
			//rDAC.Init();
			//TimeSync.setBeaconRate(numDataSamples);
			//numDataSamples = 4000;
            numDataPkts = numDataSamples / 5;
            currIndex = SensorData.startIndex;
			//currIndex = 1;
			
            for (k = 0; k < numDataPkts; k++)
            {
                // construct a TOSMSG
				/*
                packet[0] = (byte)63;
                ...
                */
                ...
                {  
                    ...
                    packet[packetIndex++] = (byte)((SensorData.Qdata[currIndex]) & 0xff);
                    currIndex++;
                    // go to beginning if rollover
                    if (currIndex == SensorData.rate * SensorData.dataBufWidth)
                        currIndex = 0;
                }
                //myRadio.Send(RadioParams.panID, RadioParams.destID, packet);
                MessageLayer.Send(0,0xffff,packet,26);
            ...
            // send completion packet so base can move on
            /*
			packet[0] = (byte)63;
            packet[1] = (byte)239;
			*/
        ...
        public static void waitUntilCommand()
        {
            int dest = 0;
            int commandType = 0;
            int receivedControlMessage = 0;
            byte[] rcvdPkt = new byte[8];

            // clear any old packets in the buffer, base station sends triplicates for reliability
            // ReceiveAny is synchronous so it will timeout and return null after the last param (100ms)
//            rcvdPkt = myRadio.ReceiveAny(ref RadioParams.rcvParam, ref RadioParams.rcvParam, 100);
//            rcvdPkt = myRadio.ReceiveAny(ref RadioParams.rcvParam, ref RadioParams.rcvParam, 100);
//            rcvdPkt = myRadio.ReceiveAny(ref RadioParams.rcvParam, ref RadioParams.rcvParam, 100);
            rcvdPkt = null;

            while (receivedControlMessage == 0)
            {
                //rcvdPkt = myRadio.ReceiveAny(ref RadioParams.rcvParam, ref RadioParams.rcvParam, 100);
                MessageLayer.Received(rcvdPkt);
				if (rcvdPkt != null)
                {
                    dest = rcvdPkt[4] * 256 + rcvdPkt[5];
                    commandType = rcvdPkt[6] * 256 + rcvdPkt[7];

                    if (commandType == 10 && dest == 255)   // noise start cmd is broadcast
                    {
                        receivedControlMessage = 1;
                        //SerialDump.print("Received noise start command");
                        StateMachine.currState = StateMachine.GETNOISE;
                    }
                    else if (commandType == 11 && dest == RadioParams.nodeID)  // download is 1-by-1
                    {
                        receivedControlMessage = 1;
                        //SerialDump.print("Received noise download command");
                        StateMachine.currState = StateMachine.DLNOISE;
                        Thread.Sleep(1000);
                    }
                    else if (commandType == 14 && dest == 255)  //bcast
                    {
                        receivedControlMessage = 1;
                        //SerialDump.print("Received detect start command");
                        StateMachine.currState = StateMachine.RUNDETECT;
                    }
                    else if (commandType == 16 && dest == RadioParams.nodeID) // 1-by-1
                    {
                        receivedControlMessage = 1;
                        //SerialDump.print("Received download data command");
            ...
            // clear duplicates
            //rcvdPkt = myRadio.ReceiveAny(ref RadioParams.rcvParam, ref RadioParams.rcvParam, 500);
            //rcvdPkt = myRadio.ReceiveAny(ref RadioParams.rcvParam, ref RadioParams.rcvParam, 500);
            //rcvdPkt = myRadio.ReceiveAny(ref RadioParams.rcvParam, ref RadioParams.rcvParam, 500);
        }
		...
		static void OnNativeEvent(uint data1, uint data2, DateTime time)
        {
			...
			if (rcvdPkt != null)
            {
				...
				dest = rcvdPkt[0] * 256 + rcvdPkt[1];
                commandType = rcvdPkt[2] * 256 + rcvdPkt[3];
              ...
              //      receivedControlMessage = 1;
                ...
                else if (commandType == 94 && dest == 255 && ((StateMachine.currState != StateMachine.RUNDETECT) || (StateMachine.currState != StateMachine.DETECTED)))  //bcast
                {
					waiting = false;
                  //  receivedControlMessage = 1;
                        //SerialDump.print("Received detect start command");
                ...
                else if (commandType == 16 && dest == RadioParams.nodeID && ((StateMachine.currState != StateMachine.DLDATA) || (StateMachine.currState != StateMachine.NOISEDONE))) // 1-by-1
                {
					waiting = false;
                    //receivedControlMessage = 1;
                        //SerialDump.print("Received download data command");
        ...
        public static void Main()
        {
            ...
            //Radio myRadio = new Radio(RadioParams._rfChannel, RadioParams._rfPower, RadioParams.panID, RadioParams.nodeID);
            //myRadio.SetRadioOption(RadioOption.LocalAddress, (ushort)RadioParams.nodeID);
            //myRadio.SetRadioOption(RadioOption.PANAddress, RadioParams.panID);

            //m_Leds.greenOn();
			
			
            // start running the main state machine, its all blocking because timers aren't jitter-free enough
			
			while (true)
            {
				//rDAC.setValue(200);
                // wait for Matlab to tell you what to do
                // the if blocks more or less match their operational order
				...
				// Yielding to other possible threads at this point
				//Thread.Sleep(10);
            }
