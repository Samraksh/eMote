...
//using Samraksh.platform.eMote;
//using Samraksh.Net.Messaging;
...
enum DETECTOR
{
    INTER_SAMP_TIME = 10, // sampling rate in ms
    SAMPRATE = 100,
    THRESH = 30,
    DC_EST_SECS = 10,
    START_TYPE = 0,
...
namespace PDRTracking
{
    public class PDRTracking
    {
		...
		public static int HALFPI = 6434;
		public static int FULLPI = 12868;
		public static int NEGPI = -12868;
		public static int TWOPI = 25736;
        ...
        // arcTan array stores the inverse tangents for [0..1] in program memory for easy lookup via findArcTan
            ...
            public static int unwrap(int aI, int aQ)
            {
                ...
                {			//1st Quadrant: arg = atan(imag/real)
                    if (a.Q <= a.I) 					// Q/I is in {0,1}
                        ...
                        newPhase = PI.HALF - findArcTan(a.I, a.Q);	// atan(x) = pi/2 - atan(1/x) for x > 0		
                }

                else if (a.I < 0 && a.Q >= 0)
                {		//2nd quadrant: arg = pi - atan(abs(imag/real)
                    if (a.Q <= absVal(a.I))
                        newPhase = PI.FULL - findArcTan(a.Q, absVal(a.I));
                    else
                        newPhase = PI.HALF + findArcTan(absVal(a.I), a.Q);  // pi - (pi/2 - atan(1/x))
                }

                else if (a.I < 0 && a.Q < 0)
                {			// 3rd quadrant: arg = -pi + atan(b/a)
                    if (absVal(a.Q) <= absVal(a.I))
                        newPhase = 0 - PI.FULL + findArcTan(absVal(a.Q), absVal(a.I));
                    else
                        newPhase = 0 - PI.HALF - findArcTan(absVal(a.I), absVal(a.Q));	// -pi + pi/2 - atan(1/x)
                }

                else if (a.I > 0 && a.Q < 0)
                {							//4th quadrant: arg = - atan(b/a)
                    if (absVal(a.Q) <= a.I)
                        newPhase = 0 - findArcTan(absVal(a.Q), a.I);
                    else
                        newPhase = 0 - PI.HALF + findArcTan(a.I, absVal(a.Q));
                }

                wPhase = newPhase;

                phase_diff = wPhase - wPhase_prev;
                if (phase_diff < PI.NEG)
                    phase_diff += PI.TWO;
                else if (phase_diff > PI.FULL)
                    phase_diff -= PI.TWO;
        ...
        public static void Main()
        {
            ...
            int ADC_SampleTime_41Cycles5 = (int)0x04;

			Debug.Print("Check Point 4\n");
            ADC.Init(ADC_SampleTime_41Cycles5);
            ...
            public static int compSampleI = 0;
			public static int compSampleQ = 0;
        ...
        public static class MessageHandler
        {
            ...
            {
                seqNum = 0;
                //payload[0] = (byte)((eMote.deviceId >> 8) & 0xff);
                //payload[1] = (byte)(eMote.deviceId & 0xff);
            }
        ...
        {
            public static OutputPort Gpio0 = new OutputPort((Cpu.Pin)0, false);
        }
            ...
            public static int[] Buff = new int[M];
            public static int state = 0;
            public static int prevstate = 0;
            public static int i, End;

            public static void MofNInit()
            {
        ...
        static void sampleFired(object o)
        {
            ...
            if (sampNum < (int)DETECTOR.SAMPRATE * (int)DETECTOR.DC_EST_SECS)
            {
                noiseSumI += currSample[0];
                noiseSumQ += currSample[1];
            }

            if (sampNum == (int)DETECTOR.SAMPRATE * (int)DETECTOR.DC_EST_SECS)
            {
                meanI = noiseSumI / ((int)DETECTOR.SAMPRATE * (int)DETECTOR.DC_EST_SECS);
                meanQ = noiseSumQ / ((int)DETECTOR.SAMPRATE * (int)DETECTOR.DC_EST_SECS);
            }
            ...
            if (sampNum >= (int)DETECTOR.SAMPRATE * (int)DETECTOR.DC_EST_SECS)
            {
				...
				//myGPIO.Gpio0.Write(true);
                compSampleI = (int)(currSample[0]) - meanI;
                compSampleQ = (int)(currSample[1]) - meanQ;

                //tempPhase = unwrap(compSampleI, compSampleQ)  >> 12;
                if (snippetIndex == 0)
                    snippetMin = snippetMax = tempPhase;
                else if (tempPhase > snippetMax)
                    snippetMax = tempPhase;
                else if (tempPhase < snippetMin)
                    snippetMin = tempPhase;

                snippetIndex++;
				
				//myGPIO.Gpio0.Write(false);

                if (snippetIndex == (int)DETECTOR.SAMPRATE) // one snippet
                {
					//myGPIO.Gpio0.Write(true);
                    if (snippetMax - snippetMin >= Thresh)
                        update(snippetNum, 1);
                    else
                        update(snippetNum, 0);
