...
namespace Microsoft.NetMicroFramework.Tools.MFDeployTool.Engine
{
    ...
    public class MFDevice : IDisposable
    {
        ...
        private void PrepareForDeploy(ArrayList blocks)
        {
            ...
            OnDebugText(this, new DebugOutputEventArgs("Getting flash map"));
        ...
        public bool Deploy(string filePath, string signatureFile, ref uint entryPoint)
        {
            ...
            OnDebugText(this, new DebugOutputEventArgs("Starting Deployment..."));
            ...
            if (blocks.Count > 0)
            {
                ...
                OnDebugText(this, new DebugOutputEventArgs("Preparing: " + Convert.ToString(blocks.Count) + "Blocks, for " + total + " bytes for Writing."));
                PrepareForDeploy(blocks);
                OnDebugText(this, new DebugOutputEventArgs("Prepare done."));
                ...
                foreach (_DBG.SRecordFile.Block block in blocks)
                {
                  ...
                  /* if (!m_eng.EraseMemory(block.address, (uint)len))
                    {
                        Console.WriteLine("Erasing memory for block: " + Convert.ToString(block.address) + "Failed.");
                        OnDebugText(this, new DebugOutputEventArgs("Erasing memory for block: " + Convert.ToString(block.address) + "Failed."));
                        return false;
                    }
                    else
                    {
                        OnDebugText(this, new DebugOutputEventArgs("Erasing memory for block: " + Convert.ToString(block.address) + "Success."));
                    }
                    */
                    OnDebugText(this, new DebugOutputEventArgs("\n\rGoing to sleep..."));
                    ...
                    while(len > 0 )
                    {
                        ...
                        {
                            Console.WriteLine("Reading file to download failed.");
                            OnDebugText(this, new DebugOutputEventArgs("Reading file to download failed."));
                            return false;
                        }
                        if (!m_eng.WriteMemory(addr, data))
                        {
                            Console.WriteLine("Writing for address: " + Convert.ToString(addr) + "Failed.");
                            OnDebugText(this, new DebugOutputEventArgs("Writing for address: " + Convert.ToString(addr) + "Failed."));
                            return false;
                        }
                        value += buflen;
                        addr += (uint)buflen;
