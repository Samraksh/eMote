...
#define RTC_MSB_Mask     ((uint32_t)0xFFFF0000)  /* RTC MSB Mask */
...
uint32_t RTC_GetDivider(void)
{
  ...
  tmp = ((uint32_t)RTC->DIVH & (uint32_t)0x000F) << 0x10;
  tmp |= RTC->DIVL;
...
ITStatus RTC_GetITStatus(uint16_t RTC_IT)
{
  ...
  bitstatus = (ITStatus)((RTC->CRL & RTC_IT) != (uint16_t)RESET);

  if (((RTC->CRH & RTC_IT) != (uint16_t)RESET) && bitstatus)
  {
    bitstatus = SET;
  }
  else
