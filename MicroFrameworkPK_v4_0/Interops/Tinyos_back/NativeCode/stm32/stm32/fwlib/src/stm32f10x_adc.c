...
/* ADC SQx mask */
#define SQR3_SQ_Set                ((uint8_t)0x1F)
#define SQR2_SQ_Set                ((uint8_t)0x1F)
#define SQR1_SQ_Set                ((uint8_t)0x1F)
...
FlagStatus ADC_GetResetCalibrationStatus(ADC_TypeDef* ADCx)
{
  ...
  if ((ADCx->CR2 & CR2_RSTCAL_Set) != (uint16_t)RESET)
  {
    /* RSTCAL bit is set */
    bitstatus = SET;
  }
...
FlagStatus ADC_GetCalibrationStatus(ADC_TypeDef* ADCx)
{
  ...
  if ((ADCx->CR2 & CR2_CAL_Set) != (uint16_t)RESET)
  {
...
FlagStatus ADC_GetSoftwareStartConvStatus(ADC_TypeDef* ADCx)
{
  ...
  if ((ADCx->CR2 & CR2_SWSTRT_Set) != (uint32_t)RESET)
  {
    /* SWSTRT bit is set */
    bitstatus = SET;
  }
...
void ADC_RegularChannelConfig(ADC_TypeDef* ADCx, uint8_t ADC_Channel, uint8_t Rank, uint8_t ADC_SampleTime)
{
  ...
  if (ADC_Channel > ADC_Channel_9)
  {
    ...
    tmpreg2 = (uint32_t)SMPR1_SMP_Set << (3 * (ADC_Channel - 10));
    /* Clear the old discontinuous mode channel count */
    tmpreg1 &= ~tmpreg2;
  ...
  else /* ADC_Channel include in ADC_Channel_[0..9] */
  {
    ...
    tmpreg1 = ADCx->SMPR2;
    /* Calculate the mask to clear */
    tmpreg2 = (uint32_t)SMPR2_SMP_Set << (3 * ADC_Channel);
    /* Clear the old discontinuous mode channel count */
    tmpreg1 &= ~tmpreg2;
    /* Calculate the mask to set */
  ...
  if (Rank < 7)
  {
    ...
    tmpreg1 = ADCx->SQR3;
    /* Calculate the mask to clear */
    tmpreg2 = (uint32_t)SQR3_SQ_Set << (5 * (Rank - 1));
  ...
  else if (Rank < 13)
  {
    ...
    tmpreg1 = ADCx->SQR2;
    /* Calculate the mask to clear */
    tmpreg2 = (uint32_t)SQR2_SQ_Set << (5 * (Rank - 7));
    /* Clear the old SQx bits for the selected rank */
    tmpreg1 &= ~tmpreg2;
    /* Calculate the mask to set */
  ...
  else
  {
    ...
    tmpreg1 = ADCx->SQR1;
    /* Calculate the mask to clear */
    tmpreg2 = (uint32_t)SQR1_SQ_Set << (5 * (Rank - 13));
    /* Clear the old SQx bits for the selected rank */
    tmpreg1 &= ~tmpreg2;
    /* Calculate the mask to set */
...
FlagStatus ADC_GetSoftwareStartInjectedConvCmdStatus(ADC_TypeDef* ADCx)
{
  ...
  if ((ADCx->CR2 & CR2_JSWSTRT_Set) != (uint32_t)RESET)
  {
    /* JSWSTRT bit is set */
    bitstatus = SET;
  }
...
void ADC_InjectedChannelConfig(ADC_TypeDef* ADCx, uint8_t ADC_Channel, uint8_t Rank, uint8_t ADC_SampleTime)
{
  ...
  tmpreg3 =  (uint8_t)((tmpreg1 & (uint32_t)~JSQR_JL_Reset)>> 20);
  /* Calculate the mask to clear: ((Rank-1)+(4-JL-1)) */
  tmpreg2 = (uint32_t)JSQR_JSQ_Set << (5 * ((Rank + 3) - (tmpreg3 + 1)));
  /* Clear the old JSQx bits for the selected rank */
  tmpreg1 &= ~tmpreg2;
  /* Calculate the mask to set: ((Rank-1)+(4-JL-1)) */
  tmpreg2 = (uint32_t)ADC_Channel << (5 * ((Rank + 3) - (tmpreg3 + 1)));
...
void ADC_SetInjectedOffset(ADC_TypeDef* ADCx, uint8_t ADC_InjectedChannel, uint16_t Offset)
{
  ...
  *((uint32_t *)((*(uint32_t*)&ADCx) + ADC_InjectedChannel)) = (uint32_t)Offset;
}
...
uint16_t ADC_GetInjectedConversionValue(ADC_TypeDef* ADCx, uint8_t ADC_InjectedChannel)
{
  ...
  return (uint16_t) (*(uint32_t*) (((*(uint32_t*)&ADCx) + ADC_InjectedChannel + JDR_Offset)));
}
