...
int USART_Driver::Write( int ComPortNum, const char* Data, size_t size )
{
	...
	//ComPortNum = 0x100; //KN setting the transport to USART mode

    if((ComPortNum < 0) || (ComPortNum >= TOTAL_USART_PORT)) {
		//LCD_DisplayStringLine(Line1,(uint8_t *)"Bad com port");
		ASSERT(FALSE); return -1;}

	if(0    == size                                        )
	{
		//LCD_DisplayStringLine(Line2,(uint8_t *)"Bad size ");
		return -1;
		}

	
    if(NULL == Data                                        ) {
			//LCD_DisplayStringLine(Line2,(uint8_t *)"Bad data ");
		ASSERT(FALSE); return -1;}
	
    HAL_USART_STATE& State = Hal_Usart_State[ComPortNum];

	//LCD_DisplayStringLine(Line0,(uint8_t *)"In fine............. ");
...
//    if ( IS_POWERSAVE_ENABLED(State) || (!IS_USART_INITIALIZED(State))) return TRUE; //KN
...

    {
        ...
        if(irq.WasDisabled() || !CPU_USART_TxBufferEmptyInterruptState( ComPortNum ) || 0 == CPU_INTC_InterruptEnableState( IrqId ))
        {
			...
			//LCD_DisplayStringLine(Line1, (uint8_t *)"irq.WasDisabled()");
            ...
            while(State.TxQueue.IsEmpty() == false)
            {
                ...
                {
//                	LCD_DisplayStringLine(Line1, (uint8_t *)"CPU_USART_TxBufferEmpty");
                    ...
                    if(!CPU_USART_TxHandshakeEnabledState(ComPortNum))
                    	{
                    	//LCD_DisplayStringLine(Line2, (uint8_t *)"CPU_USART_TxHandshakeEnabledState");
                        return FALSE;
                    	}
					//LCD_DisplayStringLine(Line1, (uint8_t *)"CPU_USART_TxBuff after");
                }
                // get next character
				
				
				//LCD_DisplayStringLine(Line2, (uint8_t *)"After RemoveCharFromTxBuffer");
				...
				//LCD_DisplayStringLine(Line3, (uint8_t *)"Before CPU_USART_WriteCharToTxBuffer");
