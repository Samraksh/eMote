...
namespace Microsoft.SPOT.Platform.Tests
{
    public class ClientTests : IMFTestInterface
    {
        ...
        public MFTestResults ClientTest_DpwsClient_Properties()
        {
            ...
            int port = 50002;
            DpwsClient testClient = new DpwsClient(port);
            try
            {

                testResult &= (testClient.EventingPort == port);

                Log.Comment("BodyParts");
                if (testClient.BodyParts != null)
                    if (testClient.BodyParts.GetType() !=
                        Type.GetType("Ws.Services.Mtom.WsMtomBodyParts"))
                        throw new Exception("BodyParts wrong type");

                testClient.BodyParts = new Ws.Services.Mtom.WsMtomBodyParts();
                if (testClient.BodyParts != null)
                    if (testClient.BodyParts.GetType() !=
                        Type.GetType("Ws.Services.Mtom.WsMtomBodyParts"))
                        throw new Exception("BodyParts wrong type after set to new");
                ...
                String uri = WsWellKnownUri.WsaAnonymousUri;
                ...
                Log.Comment("EventingPort");
                if (testClient.EventingPort.GetType() !=
                    Type.GetType("System.Int32"))
                    throw new Exception("EventingPort wrong type");
                ...
                Log.Comment("MessageType");
                if (testClient.MessageType.GetType() !=
                    Type.GetType("Ws.Services.Soap.WsMessageType"))
                    throw new Exception("MessageType wrong type");

                testClient.MessageType = new Ws.Services.Soap.WsMessageType();
                if (testClient.MessageType.GetType() !=
                    Type.GetType("Ws.Services.Soap.WsMessageType"))
                    throw new Exception("MessageType wrong type after set to new");
