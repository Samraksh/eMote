...
namespace Interop.SimpleService
{
    ...
    class SimpleServiceClient : DpwsClient
    {
        ...
        static bool m_helloRentrantFlag = false;
        ...
        void SimpleServiceClient_HelloEvent(object obj, DpwsServiceDescription helloEventArgs)
        {
            ...
            if (m_helloRentrantFlag == true)        // do not allow this method to run on more than one thread
                return;
            m_helloRentrantFlag = true;
            ...
            try
            {
                ...
                DpwsServiceDescriptions probeMatches = this.DiscoveryClient.Probe(searchTypes, 10, 10000);

                // retry
                if (probeMatches.Count == 0)
                {
                    probeMatches = this.DiscoveryClient.Probe(searchTypes, 10, 10000);
                }
        ...
        private byte[] Build2wayRequest(int X, int Y, string endpointAddress)
        {
            ...
            // Write namespaces
            xmlWriter.WriteAttributeString("xmlns", "wsa", null, WsWellKnownUri.WsaNamespaceUri_2005_08);
