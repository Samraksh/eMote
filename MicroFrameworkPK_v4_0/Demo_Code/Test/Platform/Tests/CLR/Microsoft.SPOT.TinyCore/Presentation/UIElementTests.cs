...
namespace Microsoft.SPOT.Platform.Tests
{
    public class UIElementTests : Master_Presentation, IMFTestInterface
    {
        ...
        public MFTestResults UIElement_RaiseEvent_ButtonEvent_Test1()
        {
            ...
            for (int i = 0; i < rEvents.Length; i++)
            {
                ...
                mainWindow.Dispatcher.Invoke(new TimeSpan(0, 0, 5),
                    new DispatcherOperationCallback(AddToHandler), null);
                mainWindow.Dispatcher.Invoke(new TimeSpan(0, 0, 5),
                    new DispatcherOperationCallback(RaiseEvent), null);
        ...
        public MFTestResults UIElement_AddHandlerTest3()
        {
            ...
            rEvent = Buttons.ButtonDownEvent;
            mainWindow.Dispatcher.Invoke(new TimeSpan(0, 0, 5),
                 new DispatcherOperationCallback(AddToHandler), null);
            mainWindow.Dispatcher.Invoke(new TimeSpan(0, 0, 5),
                    new DispatcherOperationCallback(RaiseEvent), null);
        ...
        public MFTestResults UIElement_AddToEventRouteTest4()
        {
            ...
            {
                rEvent = rEvents[i];
                eRoute = new EventRoute(rEvent);
                mainWindow.Dispatcher.Invoke(new TimeSpan(0, 0, 5),
        new DispatcherOperationCallback(AddToHandler), new RoutedEventArgs(rEvents[i]));
            }
            ...
            for (int i = 0; i < rEvents.Length; i++)
            {
                ...
                eRoute = new EventRoute(rEvent);
                mainWindow.Dispatcher.Invoke(new TimeSpan(0, 0, 5),
           new DispatcherOperationCallback(AddToHandler), new RoutedEventArgs(rEvent));
            }
            Log.Comment("Raising the Events and Verifying");
            for (int i = 0; i < rEvents.Length; i++)
            {
        ...
        {
            bool bRet = false;
            mainWindow.RaiseEvent(new RoutedEventArgs(rEvent, this));
        ...
        object AddToHandler(object obj)
        {
            if (btnEvent)
            {
                mainWindow.AddHandler(rEvent, new ButtonEventHandler(HandleButtonEvents), handledEventsToo);
            }
            if (fcsEvent)
            {
                mainWindow.AddHandler(rEvent, new FocusChangedEventHandler(HandleFocusChangedEvent), handledEventsToo);
            }
            if (addToEvtRoute)
            {
                mainWindow.AddToEventRoute(eRoute, obj as RoutedEventArgs);
            }
            return null;
        }

        private void HandleButtonEvents(object o, ButtonEventArgs bea)
        {
            bea.Handled = true;
            _handlerEvent.Set();
            _btnEventCntr++;
        ...
        private void HandleFocusChangedEvent(object o, FocusChangedEventArgs fcea)
        {
            fcea.Handled = true;
            _handlerEvent.Set();
            _fcsEventCntr++;
