            ...
            protected override void OnTouchDown(TouchEventArgs e)
            {
                ...
                if (calibrating)
                {
                    ...
                    if (currentCalPoint <
                        calPoints.Length)
                    {
                        cx[currentCalPoint - 1] = (short)x;
                        cy[currentCalPoint - 1] = (short)y;

                        int i = currentCalPoint - 1;
                        text.TextContent = "{(" + sx[i].ToString() + "," +
                            sy[i].ToString() + ")=(" + cx[i].ToString() + "," +
                            cy[i].ToString() + ")}";
                    }
                    else
                    {
